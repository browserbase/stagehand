...
export const createDragAndDropTool = (stagehandPage: StagehandPage) =>
  tool({
    description:
    ...
    execute: async ({ describe, startCoordinates, endCoordinates }) => {
      await stagehandPage.page.mouse.move(
        startCoordinates[0],
        startCoordinates[1],
      );
      await stagehandPage.page.mouse.down();
      await stagehandPage.page.mouse.move(endCoordinates[0], endCoordinates[1]);
