...
distribution on PyPI. The internal `just _version` recipe applies pending changesets, writes both
SDKs' release notes to the root `CHANGELOG.md`, synchronizes the Python version into
`pyproject.toml`, and refreshes `uv.lock`.
