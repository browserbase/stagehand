...
```typescript
// String instruction
await page.act(instruction: string): Promise<void>

// With ActOptions
await page.act(options: ActOptions): Promise<void>

// Execute observed action
await page.act(observeResult: ObserveResult): Promise<void>
```
...
```python
# String instruction
await page.act(instruction: str) -> None
...
# Execute observed action
await page.act(observe_result: ObserveResult) -> None
```
...
<ParamField path="action | instruction" type="string" required>
...
Resolves when action completes successfully. Throws error on failure.
