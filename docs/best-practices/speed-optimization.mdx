...
class SessionPool:
    ...
    async def get_optimized_session(self):
        if not self.sessions:
            ...
            self.sessions = await asyncio.gather(*session_tasks)
...
<CardGroup cols={1}>
<Card title="Deployments Guide" icon="rocket" href="/best-practices/deployments">
  Production patterns for session management
</Card>
</CardGroup>
...
<Info>
For cloud browser infrastructure optimization, see the [Browserbase Cost Optimization Guide](https://docs.browserbase.com/guides/cost-optimization) for additional infrastructure tuning options.
</Info>
...
<CardGroup cols={1}>
<Card title="Prompting Best Practices" icon="message" href="/best-practices/prompting-best-practices">
  Write clear, actionable prompts for better performance
</Card>
</CardGroup>
