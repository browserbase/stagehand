...
// Or execute in parallel for independent fields
await Promise.all(
  formFields.map(field => page.act(field))
);
```
```python Python
import asyncio
...
await asyncio.gather(*[
    page.act(field) for field in form_fields
])
```
</CodeGroup>
...
// For observe results (cached actions) - even shorter timeout
await page.act(cachedAction, {
  actTimeout: 2000  // Faster since no LLM inference needed
});
...
# For observe results (cached actions) - even shorter timeout
await page.act(cached_action, act_timeout=2000)
