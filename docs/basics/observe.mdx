...
- **Act on suggestions to skip LLM calls**: Pass `ObserveResult` directly to `act` (e.g., `await page.act(results[0])`) to avoid extra inference.
- **Batch for performance**: Call `observe` once (e.g., find all form fields), then `act` on each suggestion to fill/submit.
- **Cache and reuse**: Store stable `observe` results for known pages; rely on self-healing if layouts shift.
...
- **Be specific**: Narrow instructions ("Find the primary CTA in the hero") for faster, cheaper, more accurate suggestions.
- **Mind iframes and load**: If elements live in an iframe, set `iframes: true` and wait for `networkidle` before observing.
- **Use selectors to focus extraction**: Feed `selector` from `observe` into `extract` to constrain the model’s context.
...
- **Validate before critical actions**: Check the suggestion’s `method`, `selector`, and `arguments` to avoid misclicks.
- **Fallback**: If a direct `act` fails, `observe` the same prompt, verify the method, then `act` on the suggested action.
