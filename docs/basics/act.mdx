...
```
The `act()` method enables Stagehand to perform **individual** actions on a web page. Use it to build self-healing and deterministic automations that adapt to website changes. 
...
<CardGroup cols={2}>
  ...
  <Card title="Cacheing" icon="repeat" href="#reduce-model-costs">
...
With `act()`, breaking complex actions into small, single-step actions works best. If you need to orchestrate multi-step flows, use mutliple `act()` commands or [`agent()`](/fundamentals/agent/overview).
...
Here are common actions that `act()` can perform.
...
<Card title="Complete caching guide" icon="database" iconType="sharp-solid" href="/best_practices/caching">
...
<Card title="Secure browser automation guide" icon="shield-check" iconType="sharp-solid" href="/best_practices/secure-browser-agent">
...
- Use [`observe()`](/fundamentals/observe/overview) and verify the resulting action is within a list of expected actions
...
- Check if content is in iframes: [Learn more about working with iframes](/best_practices/working_with_iframes)
