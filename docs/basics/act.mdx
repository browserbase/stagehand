...
With `act()`, breaking complex actions into small, single-step actions works best. If you need to orchestrate multi-step flows, use mutliple `act()` commands or [`agent()`](/basics/agent).
...
**Don't do this - use [`agent()`](/basics/agent)**
...
<Warning>
When handling sensitive data, set `verbose: 0` in your Stagehand configuration to prevent secrets from appearing in logs. See the [configuration guide](/configuration/browser) and [secure browser agent guide](/best_practices/secure-browser-agent) for more details.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Orchestrate complex workflows with Agent" icon="robot" iconType="sharp-solid" href="/basics/agent">
   ...
   <Card title="Caching actions" icon="bolt" iconType="sharp-solid" href="/best_practices/caching">
  ...
  <Card title="Working with iframes" icon="frame" iconType="sharp-solid" href="/best_practices/working_with_iframes">
