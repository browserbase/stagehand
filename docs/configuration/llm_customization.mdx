...
- **Run any model included in AI SDK** - Find supported models in the [Vercel AI SDK](https://sdk.vercel.ai/providers/ai-sdk-providers) (Follow the guide [here](/configuration/llm_customization#vercel-ai-sdk) to get started.)
</Expandable>
...
### Best Practices
1. **Choose models**: Start with Gemini 2.5 Pro - see our [Model Evaluation](/configuration/evals) page
2. **Test models**: Evaluate performance on your specific use cases in our [Model Evaluation](/configuration/evals) guide
3. **Track costs**: Monitor token usage and set alerts using our [Observability](/configuration/observability) tools
4. **Set temperature**: 0 for automation, 0.1-0.3 for creative tasks as covered in [AI Rules](/first_steps/ai_rules)
5. **Use fallbacks**: Configure backup models following our [Best Practices](/best_practices/caching)
6. **Cache results**: Store successful patterns using our [Caching Guide](/best_practices/caching)
7. **Version models**: Track effective model versions as described in our [Contributing](/best_practices/contributing) guide
8. **Compare models**: Run A/B tests using [Custom Evals](/best_practices/creating_custom_evals)
