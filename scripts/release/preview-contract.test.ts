...
describe("preview release contract", () => {
  it("derives preview identities from stable package versions and a full commit SHA", () => {
    ...
    expect(previewTag(123, commitSha)).toBe(`preview-pr-123-${commitSha}`);
  });
  ...
  it("rejects versions and commit identities that are not stable inputs", () => {
    ...
    expect(() => previewTag(123, "0123456")).toThrow();
  });
  ...
  it("verifies every artifact against the manifest checksum", async () => {
    ...
    try {
      ...
      const repository = "browserbase/stagehand";
      const tag = previewTag(123, commitSha);
      const manifest = parsePreviewManifest({
        ...
        artifacts: {
          typescript: {
            ...
            url: releaseAssetUrl(repository, tag, "stagehand-typescript.tgz"),
            sha256: sha256(files["stagehand-typescript.tgz"]),
            install: "npm install https://example.com/stagehand-typescript.tgz",
          },
          python: {
            package: "stagehand",
            version: pythonPreviewVersion("4.0.0", commitSha),
            file: "stagehand-python.whl",
            url: releaseAssetUrl(repository, tag, "stagehand-python.whl"),
            sha256: sha256(files["stagehand-python.whl"]),
            install: "uv add https://example.com/stagehand-python.whl",
          },
          extension: {
            package: "@browserbasehq/stagehand-server",
            version: "4.0.0",
            file: "stagehand-extension.zip",
            url: releaseAssetUrl(repository, tag, "stagehand-extension.zip"),
            sha256: sha256(files["stagehand-extension.zip"]),
          },
        },
      });
      await writeFile(
        path.join(directory, `stagehand-preview-${commitSha}.json`),
