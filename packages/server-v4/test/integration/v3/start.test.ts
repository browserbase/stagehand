...
const repoRoot = (() => {
  ...
  const root = value.split("/packages/server-v4/")[0];
  if (root === value) {
...
const defaultSeaBinaryName = `stagehand-server-v4-${process.platform}-${process.arch}${process.platform === "win32" ? ".exe" : ""}`;
const seaBinaryPath = `${repoRoot}/packages/server-v4/dist/sea/${process.env.SEA_BINARY_NAME ?? defaultSeaBinaryName}`;
