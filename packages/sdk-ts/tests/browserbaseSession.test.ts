...
describe("Browserbase session creation", () => {
  it("uses web-standard requests and validates the Browserbase responses", async () => {
    ...
    await expect(requests[1]!.clone().json()).resolves.toStrictEqual({ region: "us-west-2" });
    await expect(requests[2]!.clone().json()).resolves.toStrictEqual({
      status: "REQUEST_RELEASE",
    });
  });

  it("rejects an invalid Browserbase response through the Zod schema", async () => {
    const client = createBrowserbaseApiClient("bb_key", {
      baseUrl: "https://api.browserbase.test",
