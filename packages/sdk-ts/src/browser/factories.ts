...
type BrowserFactoryDependencies = {
  ...
  createBrowserbaseSessionClient?: (apiKey: string, apiUrl: string) => BrowserbaseSessionClient;
...
function createBrowserFactories(dependencies: BrowserFactoryDependencies = {}): {
  ...
  return {
    ...
    browserbase: {
      async launch(input) {
        const { apiKey, apiUrl, ...sessionOptions } = BrowserbaseLaunchOptionsSchema.parse(input);
        return await withStagehandInitDeadline(async (signal) => {
          const sessionPromise = createBrowserbase(apiKey, apiUrl).createSession(sessionOptions);
      ...
      async connect(input) {
        ...
        const client = createBrowserbase(options.apiKey, options.apiUrl);
