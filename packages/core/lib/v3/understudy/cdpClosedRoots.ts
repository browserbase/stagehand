...
export async function collectClosedShadowRoots(
  ...
  const evaluated = await frame.session.send<Protocol.Runtime.EvaluateResponse>(
    "Runtime.evaluate",
    {
      expression: "document",
      contextId,
  ...
  if (!documentObjectId) {
    throw new Error("Unable to resolve document object");
  }

  const described = await frame.session.send<Protocol.DOM.DescribeNodeResponse>(
    "DOM.describeNode",
    {
