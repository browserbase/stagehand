...
import { waitForDomNetworkQuiet } from "../../handlers/handlerUtils/actHandlerUtils";
...
vi.mock("../../handlers/handlerUtils/timeoutGuard", () => ({
  createTimeoutGuard: vi.fn(),
}));

vi.mock("../../handlers/handlerUtils/actHandlerUtils", () => ({
  waitForDomNetworkQuiet: vi.fn(),
  performUnderstudyMethod: vi.fn(),
}));

vi.mock("../../understudy/a11y/snapshot", () => ({
...
describe("ActHandler two-step timeout", () => {
  ...
  it("throws ActTimeoutError during step 2; step 2 action does not run", async () => {
    ...
    const { performUnderstudyMethod } = await import(
      "../../handlers/handlerUtils/actHandlerUtils"
    );
    ...
    const diffCombinedTreesMock = vi.mocked(
      (await import("../../understudy/a11y/snapshot")).diffCombinedTrees,
    );
