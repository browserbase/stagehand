...
test.describe("Stagehand agent abort signal", () => {
  ...
  test("non-streaming: stagehand.close() aborts running agent tasks", async () => {
    test.setTimeout(60000);
    ...
    // Close after 500ms - this should abort the running task
    setTimeout(() => {
      v3Instance.close().catch(() => {});
    }, 500);

    await expect(executePromise).rejects.toThrow(AgentAbortError);
  });

  test("streaming: stagehand.close() stops stream and rejects result with AgentAbortError", async () => {
    test.setTimeout(60000);
    ...
    // Start a long-running streaming task
    const streamResult = await agent.execute({
      instruction:
        "Describe every visual element on this page in extreme detail. Do not use the close tool until you have described at least 100 different elements.",
      maxSteps: 50,
