...
import type { Action, ActResult, ActResultData } from "../types/public/index.js";
import type { Logger } from "../types/private/logger.js";
import type { Page } from "../understudy/page.js";
import { CacheStorage } from "./CacheStorage.js";
import { normalizeUrlForCacheKey, safeGetPageUrl, waitForCachedSelector } from "./utils.js";
import { ActCacheContext, ActCacheDeps, CachedActEntry } from "../types/private/index.js";
...
export class ActCache {
  ...
  async tryReplay(
    ...
    if (
      ...
      (!context.variables || !this.hasAllVariableValues(contextVariableKeys, context.variables))
    ) {
      this.logger({
        category: "cache",
  ...
  private async replayCachedActions(
    ...
    const execute = async (): Promise<ActResult> => {
      const actionResults: ActResultData[] = [];
  ...
  private async refreshCacheEntry(context: ActCacheContext, entry: CachedActEntry): Promise<void> {
  ...
  private doVariableKeysMatch(entryKeys: string[], contextKeys: string[]): boolean {
  ...
  private hasAllVariableValues(variableKeys: string[], variables: Record<string, string>): boolean {
