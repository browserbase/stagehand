...
export async function waitForInputEventsToSettle(session: CDPSessionLike): Promise<void> {
  try {
...
export async function waitForInputEventsToSettleInSessions(
  ...
  await Promise.all([...new Set(sessions)].map((session) => waitForInputEventsToSettle(session)));
}
...
function formatExceptionDetails(details: Protocol.Runtime.ExceptionDetails): string {
  return (
