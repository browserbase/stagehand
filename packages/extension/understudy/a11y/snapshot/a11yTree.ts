...
export function decorateRoles(
  ...
  const asRole = (n: Protocol.Accessibility.AXNode) => String(n.role?.value ?? "");
...
export async function buildHierarchicalTree(
  ...
  for (const n of nodes) {
    const keep =
      !!(n.name && n.name.trim()) || !!(n.childIds && n.childIds.length) || !isStructural(n.role);
    if (!keep) continue;
  ...
  const cleaned = (await Promise.all(roots.map(pruneStructuralSafe))).filter(Boolean) as A11yNode[];
  ...
  async function pruneStructuralSafe(node: A11yNode): Promise<A11yNode | null> {
    ...
    const cleanedKids = (await Promise.all(children.map(pruneStructuralSafe))).filter(
      Boolean,
    ) as A11yNode[];
...
export function extractUrlFromAXNode(ax: Protocol.Accessibility.AXNode): string | undefined {
...
function extractBooleanProperty(
  ...
  const value = node.properties?.find((p) => p.name === propertyName)?.value?.value;
