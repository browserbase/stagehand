...
/** Keep every candidate from the existing top 30, then add BM25's best outsider. */
export function shortlistWithBm25(
  nodes: OutlineNode[],
  candidates: OutlineNode[],
  instruction: string,
  keep = 30,
): OutlineNode[] {
  const select = (scores: Map<string, number>, limit: number) =>
    candidates
      .filter((candidate) => (scores.get(candidate.id) ?? 0) > 0)
      .sort((a, b) => (scores.get(b.id) ?? 0) - (scores.get(a.id) ?? 0) || a.index - b.index)
      .slice(0, limit);
  const baseline = select(scoreCandidates(nodes, candidates, instruction), keep);
  if (baseline.length < keep) return baseline.sort((a, b) => a.index - b.index);
  const selected = new Set(baseline.map((candidate) => candidate.id));
  const bestBm25 = select(scoreCandidatesBm25(nodes, candidates, instruction), 1)[0];
  if (bestBm25 && !selected.has(bestBm25.id)) baseline.push(bestBm25);
  return baseline.sort((a, b) => a.index - b.index);
}
