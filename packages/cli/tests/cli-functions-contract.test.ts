  ...
  it("shuts down if the runtime process already exited", async () => {
    ...
    await withServer(
      ...
      async ({ baseUrl }) => {
        ...
        // A runtime that connected and then exited must not remain reported as
        // healthy while the dev server waits for a replacement process.
        await waitForStdout(child, '"runtimeConnected": false', 15_000);
        child.kill("SIGTERM");
