...
describe("secret CLI HTTP contracts", () => {
  it.each(["stdin", "env"])(
    ...
    async (source) => {
      ...
      server = await startFakeBrowserbaseServer((request, response) => {
        ...
        else jsonResponse(response, 201, metadata);
      });
      ...
      expect(Object.keys(body).sort()).toEqual(
        ["secretKey", "sealedSecretValue", "keypairId"].sort(),
      );
      expect(body.secretKey).toBe("SERVICE_TOKEN");
  ...
  it("reports a duplicate key without retrying creation", async () => {
    const { publicKey } = await keypair();
    server = await startFakeBrowserbaseServer((request, response) => {
      if (request.path === "/v1/secrets/keypair") {
        jsonResponse(response, 200, { id: "keypair-1", publicKey });
      } else {
        jsonResponse(response, 409, { message: "Secret already exists" });
      }
    });
    const result = await runCli(
      [
    ...
    expect(result.exitCode).not.toBe(0);
    expect(result.stderr).toContain("Secret already exists");
