...
export type DriverPage = NonNullable<Awaited<ReturnType<DriverContext["activePage"]>>>;
...
export class DriverSessionManager {
  ...
  private releaseRemote: (() => Promise<void>) | null = null;
  ...
  async close(): Promise<void> {
    ...
    const releaseRemote = this.releaseRemote;
    this.stagehand = null;
    this.browser = null;
    this.context = null;
    ...
    if (releaseRemote) {
      await releaseRemote().catch(() => undefined);
    }
  }
  ...
  private async ensurePage(options: { createIfMissing?: boolean } = {}): Promise<DriverPage> {
    ...
    const existingPage = (await this.activePageIfPresent()) ?? (await this.context.pages())[0];
    if (existingPage) {
  ...
  private async initialize(): Promise<void> {
    ...
    try {
      if (resolvedTarget.kind === "remote") {
        ...
        browser = await localBrowser.connect({ cdpUrl: resolvedTarget.endpoint });
      } else {
      ...
      this.releaseRemote = releaseRemote ?? null;
