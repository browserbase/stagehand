...
class BrowserbaseUnlistedModelConfig(WireModel):
    model_config = ConfigDict(
        extra="forbid",
        validate_by_name=True,
    )
    type: Literal["unlisted"]
    model_name: BrowserbaseUnlistedModelName


class BrowserbaseUnlistedModelName(RootModel[StrictStr]):
    root: Annotated[
        StrictStr,
        Field(
            pattern="^browserbase\\/(openai|anthropic|google|groq|cerebras)\\/[\\s\\S]{1,}$"
        ),
    ]
...
class UnlistedModelConfig(WireModel):
    model_config = ConfigDict(
        extra="forbid",
        validate_by_name=True,
    )
    ...
    """Custom headers sent with every request to the model provider"""
    type: Literal["unlisted"]
    model_name: UnlistedModelName


class ModelConfig(
    RootModel[Union[KnownModelConfig, UnlistedModelConfig, BrowserbaseModelConfig, BrowserbaseUnlistedModelConfig, CustomModelConfig]]
):
    root: Union[KnownModelConfig, UnlistedModelConfig, BrowserbaseModelConfig, BrowserbaseUnlistedModelConfig, CustomModelConfig]


class UnlistedModelName(RootModel[StrictStr]):
    root: Annotated[
        StrictStr,
        Field(pattern="^(openai|anthropic|google|groq|cerebras)\\/[\\s\\S]{1,}$"),
    ]
    """A provider-qualified model name forwarded without Stagehand catalog metadata."""
