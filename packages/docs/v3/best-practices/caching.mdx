...
The cache key is generated from the instruction, page content, and options you pass. On a cache hit, the response is returned directly from the server — no LLM inference, no token cost. You can inspect cache behavior via the `cacheStatus` field returned by `act()` and `extract()`. Check out the [Browserbase blog](https://www.browserbase.com/blog/stagehand-caching) for more details on how it works under the hood.
...
<Note>
  `observe()` does not currently expose a `cacheStatus` field. To check
  whether an `observe()` call was served from cache, use the [Browserbase
  session replay
  dashboard](https://docs.browserbase.com/features/observability#stagehand) or
  inspect the session logs.
</Note>

### Limitations

- Browserbase Cache does **not** apply to `agent()` at the moment. Use Local Cache (below) if you need caching for agent workflows.
...
Local Cache writes action results to your filesystem so they persist across script runs. It works in both `LOCAL` and `BROWSERBASE` environments. When you specify a `cacheDir`, Stagehand saves every action and agent step to a local file on first run, then replays those cached actions on subsequent runs — no LLM calls, no token cost, and no network round-trip to Browserbase.

This is especially useful for:

- **`agent()` workflows** — Browserbase Cache doesn't cover `agent()` yet, so local caching is the only way to cache agent steps
