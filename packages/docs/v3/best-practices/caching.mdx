...
<Note>
  `observe()` does not currently expose a `cacheStatus` field. To check whether an `observe()` call was served from cache, use the [Browserbase session replay dashboard](https://www.browserbase.com/sessions) or inspect the session logs.
</Note>
