...
Possible reasons include `threshold` (a cache entry exists but hasn't been seen enough times to be served yet — see [Tune the cache threshold](#tune-the-cache-threshold)), `empty_array`, `timeout`, and `error`. A first-time miss with no prior entry has no `cacheMissReason`. The field is available on `act()`, `extract()`, and `observe()` results.
