...
// When setting the API key required for authentication in the request header
const deepWikiClient = await connectToMCPServer({
  serverUrl: "https://mcp.devin.ai/mcp",
  requestOptions: {
    requestInit: {
      headers: {
        Authorization: `Bearer ${process.env.DEEP_WIKI_API_KEY}`,
      },
    },
  },
});
...
const agent = stagehand.agent({
  provider: "openai", 
  model: "computer-use-preview",
  integrations: [supabaseClient, deepWikiClient, notionClient],
