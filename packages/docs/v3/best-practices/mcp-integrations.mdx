...
const searchClient = await connectToMCPServer(/* search config */);
...
```typescript
const agent = stagehand.agent({
  integrations: [`https://mcp.database-tools.com/mcp?apiKey=${process.env.DATABASE_API_KEY}`],
  systemPrompt: `You have access to database tools. Use them to store and retrieve data.`
});
```
...
2. Database tools - Use to store/retrieve data. Don't use the \`execute_sql\` tool directly. Use the \`query\` tool instead.
3. Browser automation - Use for web interactions. Only use the \`navigate\` tool to navigate to a URL, use \`goBack\` and \`goForward\` to navigate back and forward in the browser history.
