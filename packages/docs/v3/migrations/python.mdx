...
In v3, LLM metrics are no longer retrieved via a separate replay endpoint. Instead, they are included directly in the `usage` field of operation responses.

#### Metric Name Changes

| Old SDK (v2) | New SDK (v3) | Description |
|--------------|--------------|-------------|
| `act_prompt_tokens` | `input_tokens` | Tokens sent to the LLM |
| `act_completion_tokens` | `output_tokens` | Tokens generated by the LLM |
| `act_inference_time_ms` | `inference_time_ms` | Time taken for the LLM call |
| *N/A* | `cached_input_tokens` | Tokens served from cache |
| *N/A* | `reasoning_tokens` | Internal tokens used for reasoning |
...
```python
# Metrics were accessed via stagehand.metrics or a replay endpoint
metrics = stagehand.metrics
print(f"Prompt tokens: {metrics['act_prompt_tokens']}")
print(f"Completion tokens: {metrics['act_completion_tokens']}")
print(f"Inference time: {metrics['act_inference_time_ms']} ms")
```
...
```python
# Metrics are now in the response.data.result.usage field
act_response = client.sessions.act(
    id=session_id,
...
if act_response.data.result.usage:
    usage = act_response.data.result.usage
    ...
    if usage.cached_input_tokens:
        print(f"Cached tokens: {usage.cached_input_tokens}")
...
async with AsyncStagehand() as client:
    session = await client.sessions.create(model_name="openai/gpt-4o-mini")

    response = await session.execute(
        execute_options={"instruction": "find contact info"},
        agent_config={"model": "openai/gpt-4o-mini"}
    )

    # Access metrics via response.data.result.usage
...
<Note>
The `usage` field is available on responses from `act()`, `observe()`, `extract()`, and `execute()` operations.
</Note>
...
| `stagehand.metrics` | `response.data.result.usage` (per-operation) |
...
<AccordionGroup>
  ...
  <Accordion title="LLM metrics endpoint returns 404">
