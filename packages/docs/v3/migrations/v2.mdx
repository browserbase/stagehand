...
- **Removing Playwright Dependency**: Stagehand v3 is now a standalone library that does not depend on Playwright. **You can still use Stagehand with Playwright, check out our [Playwright integration](/integrations/playwright) for more details.**
...
```typescript v2 icon="/images/typescript.svg"
...
```typescript v3 icon="/images/typescript.svg"
import { Stagehand } from "@browserbasehq/stagehand";
...
// automatically takes action on the active page
...
// Use browser DevTools or logging for debugging
...
**Problem**: Direct `stagehand.page` is not supported in v3.
...
const agent = stagehand.agent({
  model: "google/gemini-2.5-computer-use-preview-10-2025",
  systemPrompt: "You are helpful. Additional context here."  // Move context to systemPrompt
});
...
3. **Use the Context API** for accessing pages
