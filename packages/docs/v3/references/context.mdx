...
await stagehand.stagehand.act("click the export button");

// Switch to another page
context.setActivePage(settingsPage);
await stagehand.stagehand.act("enable notifications");

// Back to main page
context.setActivePage(mainPage);
await stagehand.stagehand.act("click the logout button");
...
const results = await Promise.all([
  page1.extract("get page info", schema),
  page2.extract("get page info", schema),
  page3.extract("get page info", schema)
]);
...
// stagehand.page now refers to homePage
await stagehand.stagehand.act("click the header link");
...
## Integration with Stagehand.page

The `stagehand.page` property returns the currently active page from the context:

```typescript
const stagehand = new Stagehand({ env: "LOCAL" });
await stagehand.init();

// stagehand.page === context.activePage()
const page = stagehand.context.pages()[0];

// Create a new page - it becomes active
const newPage = await stagehand.context.newPage();

// Now stagehand.page === newPage
...
- `stagehand.page` always returns `context.activePage()`
...
3. **Set active page before operations** - Ensure the correct page is active before calling `stagehand.page` methods
...
await stagehand.stagehand.act("view report");

context.setActivePage(pages.settings);
await stagehand.stagehand.act("update preferences");
```
...
const data = await Promise.all(
  pages.map(page => stagehand.extract("get data", schema))
);
```
...
if (needsDashboard) {
  ...
  await dashboard.act("generate report");
}
