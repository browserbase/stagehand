...
await stagehand.page.act("click the export button");

// Switch to another page
context.setActivePage(settingsPage);
await stagehand.page.act("enable notifications");

// Back to main page
context.setActivePage(mainPage);
await stagehand.page.act("click the logout button");
...
await stagehand.page.act("click the header link");
...
await stagehand.page.act("view report");

context.setActivePage(pages.settings);
await stagehand.page.act("update preferences");
```
...
const data = await Promise.all(
  pages.map(page => page.extract("get data", schema))
);
```
