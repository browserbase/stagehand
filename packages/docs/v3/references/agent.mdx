...
<ParamField path="mode" type='"dom" | "hybrid" | "cua"' optional>
  ...
  **Modes:**
  - `"dom"` (default): Uses DOM-based tools (`act`, `actOnElement`, `fillForm`) for structured page interactions. Works with any model.
  - `"hybrid"`: Uses both DOM-based and coordinate-based tools (`act`, `actOnElement`, `click`, `type`, `hover`, `dragAndDrop`, `clickAndHold`, `fillFormVision`) for visual/screenshot-based interactions. Requires models with reliable coordinate-based action capabilities.
...
<ParamField path="excludeTools" type="string[]" optional>
  ...
  **DOM mode (default):** `act`, `actOnElement`, `fillForm`, `ariaTree`, `extract`, `goto`, `scroll`, `keys`, `navback`, `screenshot`, `think`, `wait`, `search`

  **Hybrid mode:** `click`, `type`, `hover`, `dragAndDrop`, `clickAndHold`, `fillFormVision`, `act`, `actOnElement`, `ariaTree`, `extract`, `goto`, `scroll`, `keys`, `navback`, `screenshot`, `think`, `wait`, `search`
