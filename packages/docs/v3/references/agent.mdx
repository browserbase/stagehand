...
interface AgentExecuteOptions {
  ...
  variables?: Variables; // Pass sensitive data securely (experimental)
...
<ParamField path="variables" type="Variables" optional>
  Pass sensitive data (passwords, API keys, personal information) to the agent without exposing actual values to the LLM. The agent receives only variable names and descriptions, using `%variableName%` syntax in tool calls.

  **Variable Interface:**
  ```typescript
  type VariableValue = string | number | boolean;
  interface Variable {
    value: VariableValue;      // The actual value (never sent to LLM)
  ...
  **Example:**
  ```typescript
  variables: {
    username: { value: "john@example.com", description: "User's email" },
    password: { value: process.env.PASSWORD, description: "User's password" }
  }
  ```

  **Supported tools:** `act`, `fillForm` (DOM mode), `type`, `fillFormVision` (Hybrid mode)
...
// Pass sensitive data securely using variables
const result = await agent.execute({
  ...
  variables: {
    ...
    password: {
      value: process.env.USER_PASSWORD, // Actual value never sent to LLM
