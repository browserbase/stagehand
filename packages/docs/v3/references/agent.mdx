...
<ParamField path="tools" type="ToolSet" optional>
  Custom tool definitions to extend agent capabilities. Use the `tool` helper exported from `@browserbasehq/stagehand`.
</ParamField>
...
interface AgentExecuteOptions {
  ...
  variables?: Variables; // Secure variable substitution (experimental)
...
<ParamField path="variables" type="Variables" optional>
  Named variables for secure data substitution. The agent sees variable names and descriptions in its system prompt but never the actual values. Values are substituted at runtime using `%variableName%` syntax.

  Each variable should be an object with `value` and `description`. The `description` helps the agent understand the purpose of the variable and when to use it.
  ...
  <Expandable title="VariableValue Type">
    Each variable value can be:
    - An object with `value` and optional `description` (recommended for agents)
    - A simple primitive (`string`, `number`, `boolean`)
...
const result = await agent.execute({
  instruction: "Log into the website and navigate to account settings",
  maxSteps: 15,
  variables: {
    username: {
      value: "john@example.com",
    ...
    totp: {
      value: generateTOTP(process.env.TOTP_SECRET),
      description: "One-time 2FA code for the verification step"
    }
  }
});

console.log("Logged in:", result.completed);
```
...
- **ExperimentalNotConfiguredError** - Thrown when experimental features (callbacks, signal, messages, streaming, variables) are used without `experimental: true` in Stagehand constructor