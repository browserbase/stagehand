...
Reveals the protocol reported by the browser (e.g. `https`, `http`, `data`, `about`). Useful for distinguishing non-network navigations.
...
Each method resolves with `StagehandResponse | null` depending on whether Chrome reported a document-level network response.
