...
await stagehand.extract(instruction: string, options?: ExtractOptions): Promise<{ extraction: string }>
...
const product = await stagehand.extract("extract product details", ProductSchema);
```
