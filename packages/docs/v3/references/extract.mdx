...
// Options only (for page text with custom options)
await page.extract(options: ExtractOptions): Promise<{ pageText: string }>

// String instruction only
await page.extract(instruction: string, options?: ExtractOptions): Promise<{ extraction: string }>

// With schema
await page.extract<T extends ZodTypeAny>(
  instruction: string,
  schema: T,
...
<ParamField path="model" type="ModelConfiguration" optional>
  Configure the AI model to use for this extraction. Can be either:
...
<ParamField path="selector" type="string" optional>
  Optional XPath selector to limit extraction scope to a specific part of the page. Reduces token usage and improves accuracy.
</ParamField>
...
const product = await page.extract("extract product details", ProductSchema);
```
...
// Using string format
const data1 = await page.extract(
...
const data2 = await page.extract(
  "extract article data",
  DataSchema,
  {
    model: {
...
const data1 = await page.extract("get title", Schema, { page: page1 });
const data2 = await page.extract("get title", Schema, { page: page2 });
```
