...
Access the current active page for automation.
...
- Interaction (`click()`, `type()`, `locator()`)
- Inspection (`url()`, `title()`, `screenshot()`)
- JavaScript evaluation (`evaluate()`)
- **AI-powered methods** ([`act()`](/v3/references/act), [`extract()`](/v3/references/extract), [`observe()`](/v3/references/observe))
...
await page.goto("https://example.com");
const data = await page.extract("get page title", z.object({
  title: z.string()
}));
...
// Now stagehand.page returns page2
...
await page.extract("get form data", schema);
