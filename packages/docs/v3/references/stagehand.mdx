...
<ParamField path="useTouch" type="boolean" optional>
  ...
  <Note>Explicit opt-in only — nothing is derived from the session. Set this whenever the session renders a mobile layout, e.g. a Browserbase session created with `browserSettings.os: "mobile"`.</Note>
