...
<ParamField path="useTouch" type="boolean" optional>
  ...
  When omitted, this is derived from the session: a Browserbase session with `browserSettings.os` of `"mobile"` or `"tablet"`, or a local session launched with `localBrowserLaunchOptions.hasTouch`.

  <Note>Set this explicitly to opt in when resuming a session via `browserbaseSessionID`, since the original `browserSettings` are not known to the SDK in that case.</Note>

  **Default:** derived from the session (see above)
</ParamField>
