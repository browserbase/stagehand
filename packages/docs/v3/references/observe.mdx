...
await page.observe(instruction: string): Promise<Action[]>

// String instruction with options
await page.observe(instruction: string, options: ObserveOptions): Promise<Action[]>
```
...
const elements = await page.observe("find important call-to-action buttons", {
  model: "openai/gpt-4o",
  timeout: 45000
});
...
#### Focused Observation with Selector

```typescript
// Focus observation on a specific part of the page
const tableActions = await page.observe("find all table rows", {
...
const page1Actions = await page.observe("find navigation", { page: page1 });
const page2Actions = await page.observe("find buttons", { page: page2 });
```
