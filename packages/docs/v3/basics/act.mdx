...
// Second run (or subsequent script executions) - reuses cached action (no LLM call)
await page.act("click the login button");
```

<Note>
Caching persists across script executions. The first time you run your script, actions are cached to disk. On subsequent runs, cached actions are reused automatically, significantly reducing costs and improving performance.
</Note>
