...
## Return value of `act()`?
When you use `act()`, Stagehand will return a `Promise<ActResult>` with the following properties:
``` typescript
{
  success: true,
...
await stagehand.act("choose 'Peach' from the favorite colour dropdown", {
  model: {
    provider: "google",
