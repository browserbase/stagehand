...
// Type: Array<{ address: string; price: string; sqft: number }>
```
...
<Tab title="With Schema (Primitive)">
...
```typescript
const result = await page.extract("extract the repository name");
