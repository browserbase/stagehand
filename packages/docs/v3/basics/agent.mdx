...
Use variables to pass sensitive data (like passwords, API keys, or personal information) to the agent without exposing the actual values to the LLM. The agent sees only variable names and descriptions in its system prompt, while the actual values are substituted at runtime.

<Note>**Non-CUA agents only.** Variables require `experimental: true` and are not available with Computer Use Agents.</Note>

### Basic Usage

Each variable is an object with a `value` and an optional `description`. The description helps the agent understand the purpose of each variable, so it knows when and how to use them.
...
<Tip>
Always provide a `description` for agent variables. The agent relies on the description to understand what each variable is for and when to use it. Without a description, the agent only sees the variable name.
</Tip>
...
<Tab title="Do this">
```typescript
// Use variables with descriptions for sensitive data
variables: {
  apiKey: {
  ...
  accountId: {
    value: "acct_12345",
    description: "The target account identifier"
  }
}
```
</Tab>
...
// Don't omit descriptions — the agent won't know what the variable is for
variables: {
  x: "secret123"
}
```
