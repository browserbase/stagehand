...
In hybrid mode, the `screenshot` tool returns page metadata (viewport height, scroll height, element counts) alongside the image, helping the agent decide whether to call `ariaTree`. After calling `ariaTree`, the agent uses `actOnElement` with the element ID from the tree to interact directly — skipping redundant element inference for faster, more reliable actions.
...
| `actOnElement` | Act directly on an ariaTree element by ID — faster than `act` |
| `fillForm` | Fill form fields using DOM selectors |
| `ariaTree` | Get accessibility tree of the page with element IDs |
...
| `hover` | Hover over an element to reveal tooltips, menus, or hidden content |
...
| `actOnElement` | Act directly on an ariaTree element by ID — faster than `act` |
| `ariaTree` | Get accessibility tree with element IDs |
