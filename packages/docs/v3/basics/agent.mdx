...
`stagehand.agent()` returns an `AgentInstance` with two methods:

```typescript
const agent = stagehand.agent();

await agent.execute("Fill out the signup form"); // -> Promise<AgentResult>

agent.stop(); // -> boolean (true when an active run was halted)
```

`agent.execute()` resolves to an `AgentResult` like the example below:
