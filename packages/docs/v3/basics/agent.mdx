...
When the user already knows the local file path, the agent can upload it natively as part of the workflow without manually calling `setInputFiles()`.
...
await agent.execute({
  instruction:
    'Upload the file at "/Users/me/Documents/resume.pdf" to the resume file input, then continue the application flow.',
  maxSteps: 10,
});
```

Use the actual local path in the instruction. The agent will use its built-in `upload` tool instead of clicking the visible upload button.
...
| `upload` | Use `%variableName%` in file paths |
