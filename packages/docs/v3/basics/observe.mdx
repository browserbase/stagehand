...
// Discover actions
const actions = await page.observe("find the login form fields");
console.log(actions);
// [
...
if (loginButton) {
  ...
  await page.act(loginButton); // No LLM call needed!
}
```
...
const [dataSection] = await page.observe("find the main data table");
...
const [button] = await page.observe("find the submit button");
await page.act(button);

// Good: Focus extraction
const [table] = await page.observe("find the data table");
await page.extract({ instruction: "extract rows", selector: table.selector });
```
...
const results = await page.observe("what is the page title?");
...
await page.observe("find stuff");
await page.observe("find things to click");
```
...
**Example:**
```typescript
const actions = await page.observe("find login form fields");
console.log(actions);
// [
//   {
