...
- **Action caching** — Model Gateway works with Stagehand's [managed action caching](/v3/best-practices/server-caching), so repeated steps are reused instead of re-run.
...
<Tip>
  ...
  feature](/v3/best-practices/server-caching).
</Tip>
...
- Implement [caching](/v3/best-practices/server-caching) for repeated patterns
</Accordion>
...
<Card title="Caching Guide" href="/v3/best-practices/server-caching" icon="database">
