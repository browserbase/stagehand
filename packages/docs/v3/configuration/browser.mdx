...
<Card title="Complete Caching Guide" icon="database" href="/best-practices/caching">
  Learn advanced caching strategies and best practices.
</Card>

### Self-Healing Actions

Enable automatic retry and recovery for failed actions:

```typescript
const stagehand = new Stagehand({
  env: "BROWSERBASE",
  selfHeal: true, // Automatically retry failed actions with alternative strategies
});
```

When `selfHeal` is enabled, Stagehand will:
- Retry actions with different selectors if the initial attempt fails
- Adapt to minor page layout changes
- Use alternative strategies to complete actions
...
const stagehand = new Stagehand({
  env: "BROWSERBASE",
  domSettleTimeout: 3000, // Wait up to 3 seconds for DOM to settle (default varies by operation)
});
```
...
Enable experimental features that are under development:
...
<Warning>
Experimental features may change or be removed in future versions. Use with caution in production environments.
</Warning>
