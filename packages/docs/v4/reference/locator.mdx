...
Set one or more local files on the matched `<input type="file">`. Relative paths are resolved on the SDK caller's machine. Pass an empty array to clear the selection.
...
<ParamField path="files" type="FileInput">
  Local file path(s) or in-memory payload(s) to upload.
  ...
  <ParamField path="files.data" type="string">
    The base64-encoded file data sent over the protocol after SDK normalization.
  </ParamField>
...
Set one or more local files on the matched `<input type="file">`. Relative paths are resolved on the SDK caller's machine. Pass an empty sequence to clear the selection.
...
<ParamField path="files" type="str | Path | FilePayload | Sequence[str | Path | FilePayload]">
  ...
  <ParamField path="files.data" type="str">
    The base64-encoded file data sent over the protocol after SDK normalization.
  </ParamField>
