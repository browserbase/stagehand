...
Set one or more local files or in-memory payloads on the matched `<input type="file">`. Relative paths are resolved on the SDK caller's machine. Pass an empty array to clear the selection.
...
<ParamField path="files" type="FileInput">
  Local file path(s) or in-memory payload(s) to upload. Files are serialized in memory and limited to 50 MiB each.
...
Set one or more local files or in-memory payloads on the matched `<input type="file">`. Relative paths are resolved on the SDK caller's machine. Pass an empty sequence to clear the selection.
