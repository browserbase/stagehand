...
While no policy is set, this returns `null` and the context enforces the default posture described in `setDomainPolicy()`.
...
A context starts with a default posture that blocks cloud metadata and link-local endpoints, so agent-driven navigation cannot reach them by accident. The default blocks `169.254.0.0/16`, IPv6 link-local addresses that begin with `fe80:`, IPv4-mapped IPv6 spellings, and the metadata endpoints `fd00:ec2::254`, `100.100.100.200`, `metadata.google.internal`, and `metadata.goog`. Blocked requests fail with Chrome's `BlockedByClient` error.

Setting a policy replaces the default posture. To reach a host the default blocks, name it in `allowedDomains`:

```typescript
await browser.context.setDomainPolicy({ allowedDomains: ["169.254.1.5", "example.com"] });
```

An allowlist blocks every host you do not name, and a policy with only `blockedDomains` replaces the default list. Domain patterns match one host, and only a leading `*.` matches subdomains, so a link-local range or an IPv6 endpoint is not expressible as a pattern.

Passing `null` clears your policy and returns the context to the default posture.
...
While no policy is set, this returns `None` and the context enforces the default posture described in `set_domain_policy()`.
...
Setting a policy replaces the default posture. To reach a host the default blocks, name it in `allowed_domains`:

```python
await browser.context.set_domain_policy({"allowed_domains": ["169.254.1.5", "example.com"]})
```

An allowlist blocks every host you do not name, and a policy with only `blocked_domains` replaces the default list. Domain patterns match one host, and only a leading `*.` matches subdomains, so a link-local range or an IPv6 endpoint is not expressible as a pattern.

Passing `None` clears your policy and returns the context to the default posture.
...
While no policy is set, this returns `nil` and the context enforces the default posture described in `SetDomainPolicy()`.
...
if err := browserContext.SetDomainPolicy(ctx, &stagehand.DomainPolicy{
    AllowedDomains: []string{"169.254.1.5", "example.com"},
}); err != nil {
    return err
}
```
...
Passing `nil` clears your policy and returns the context to the default posture.
