...
Export cookies for this context in a Playwright-compatible storage state shape. The method reads cookies from CDP, so it includes HttpOnly session cookies. The method currently returns `origins` as `[]` and reserves it for future localStorage support.
...
<ResponseField name="result" type="Promise<StorageState>">
  ...
  <ResponseField name="result.origins" type="StorageStateOrigin[]">
    The method currently returns an empty array and reserves this field for future origin-scoped localStorage export.
  </ResponseField>
</ResponseField>

## setStorageState()

Replace cookies in this context from a storage state object or JSON file. The method clears existing cookies first. Stagehand ignores entries under `origins` until localStorage support lands.
...
Export cookies for this context in a Playwright-compatible storage state shape. The method reads cookies from CDP, so it includes HttpOnly session cookies. The method currently returns `Origins` as empty and reserves it for future localStorage support.
...
```go
_, err := browserContext.StorageState(ctx, &stagehand.StorageStateOptions{Path: "auth.json"})
if err != nil {
    return err
}
```
...
<ResponseField name="result" type="StorageState">
  ...
  <ResponseField name="result.Origins" type="[]StorageStateOrigin">
    The method currently returns an empty slice and reserves this field for future origin-scoped localStorage export.
  </ResponseField>
</ResponseField>

## SetStorageState()

Replace cookies in this context from a storage state object. The method clears existing cookies first. Stagehand ignores entries under `Origins` until localStorage support lands.
...
```go
if err := browserContext.SetStorageState(ctx, state); err != nil {
    return err
}
```

<ParamField path="state" type="StorageState">
  A storage state object whose cookies replace the current cookie jar.
</ParamField>
...
Replace cookies in this context from a Playwright-compatible JSON file. The method clears existing cookies first. Stagehand ignores entries under `Origins` until localStorage support lands.
...
<ParamField path="path" type="string">
  A path to a Playwright-compatible JSON file.
</ParamField>
