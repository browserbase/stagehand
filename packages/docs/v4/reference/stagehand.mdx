...
Each Stagehand instance starts at zero. Only operations that return `metadata.usage` count toward totals, so deterministic actions, cache hits, and calls that throw before returning a result leave the counters unchanged. Reading metrics does not reset them.
...
<ResponseField name="result.metadata" type="StagehandResultMetadata">
  ...
  <ResponseField name="result.metadata.usage" type="StagehandResultUsage" optional>
    Aggregate LLM usage for the operation.
...
Return token usage and inference timing metrics for this session.
Each Stagehand instance starts at zero. Only operations that return `metadata.usage` count toward totals, so deterministic actions, cache hits, and calls that raise before returning a result leave the counters unchanged. Reading metrics does not reset them.
