...
<ParamField path="logging" type="StagehandClientLoggingConfig" optional>
  Client-side log level, output format, optional console output (enabled by default), and log callback. Disabling routine console output preserves eligible callbacks and existing callback-failure diagnostics. See [Logging](/v4/configuration/logging#log-destinations).
</ParamField>
