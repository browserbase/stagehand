...
<ParamField path="options" type="StagehandClientObserveOptions" optional>
  ...
  <ParamField path="options.cache" type="Caching" optional>
    Override server-side caching for this request. Requests with `locator` or `ignoreLocators` include them in the cache key, so calls that differ only in their locators use separate cache entries.
...
<ParamField path="cache" type="bool | CacheOptions | None" optional>
  Override server-side caching for this request. Requests with `locator` or `ignore_locators` include them in the cache key, so calls that differ only in their locators use separate cache entries.
