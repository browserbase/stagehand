...
Each Stagehand instance starts at zero. Only operations that return `metadata.usage` are added, so deterministic actions, cache hits, and calls that throw before returning a result leave the counters unchanged. Reading metrics does not reset them.
...
Return token usage and inference timing metrics for this session.
Each Stagehand instance starts at zero. Only operations that return `metadata.usage` are added, so deterministic actions, cache hits, and calls that raise before returning a result leave the counters unchanged. Reading metrics does not reset them.
