...
<ParamField path="options.ignoreLocators" type="Locator[]" optional>
  Page-created locators to exclude from consideration. Create each with `page.locator("...")`; use `.nth(index)` on a locator to exclude only that indexed match.
</ParamField>

  <ParamField path="options.locator" type="Locator" optional>
    A page-created locator that scopes the operation. Create it with `page.locator("...")`; use `.nth(index)` on the locator to target a specific match.
  </ParamField>
