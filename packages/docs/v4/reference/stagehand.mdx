...
<ParamField path="schema" type="StagehandSchema | StagehandClientExtractOptions" optional>
  ...
  `toStandardJsonSchema()`, wrap Effect with both of its Standard Schema adapters, or wrap a plain
  Draft 2020-12 document with `jsonSchema()`. Defaults to an object with a string `extraction` field
...
JSON Schema. Use `jsonSchema<T>()` when the schema is already a plain Draft 2020-12 document.
...
so supply one with `jsonSchema<T>()` or omit the generic when the type is unknown. The wrapper does
...
Do not pass a raw JSON Schema document directly to `extract()`. Wrap it with `jsonSchema()` first.
