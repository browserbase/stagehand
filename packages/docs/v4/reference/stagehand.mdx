...
<ParamField path="schema" type="StagehandSchema | StagehandClientExtractOptions" optional>
  ...
  `toStandardJsonSchema()`, wrap Effect with both of its Standard Schema adapters, or wrap a JSON
  Schema object with `jsonSchema()`. Stagehand supplies the Draft 2020-12 dialect. Defaults to an object with a string `extraction` field
  when omitted. See [supported TypeScript schema implementations](#supported-typescript-schema-implementations).
</ParamField>
...
`extract()` accepts Standard Schema implementations that validate values and produce Draft 2020-12
JSON Schema. Use `jsonSchema<T>()` when you already have a JSON Schema object. Stagehand supplies the Draft 2020-12 dialect.
...
| Plain JSON Schema | Pass the object to `jsonSchema<T>()` | Draft 2020-12 (supplied by Stagehand) |
...
Pass a JSON Schema object to `jsonSchema()`. Stagehand supplies the Draft 2020-12 dialect declaration.
JSON Schema cannot infer a TypeScript type, so supply one with `jsonSchema<T>()` or omit the generic when the type is unknown. The wrapper does
not add validation callbacks, coerce values, run transforms, or apply `default` annotations. The
browser extension validates extracted data against the document before returning it.
</Tab>
...
Do not pass a raw JSON Schema object directly to `extract()`. Wrap it with `jsonSchema()` first.

A Standard Schema implementation that Stagehand cannot convert throws `StagehandSchemaError` before
the browser call. Data that fails its original Standard Schema validation throws
`StagehandValidationError`. Data that fails a plain JSON Schema document fails extraction inside the
browser extension.

Stagehand rejects fields that are not declared in an object schema. Set
`additionalProperties: true` to allow undeclared fields.
