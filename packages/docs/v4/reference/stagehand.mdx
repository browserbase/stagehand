...
Browser operations made by the callback use the worker-local `page` and `context` objects without
returning to the SDK process for every command.
...
<ParamField path="callback" type="ExperimentalBatchCallback&lt;Input, Result&gt;">
  A self-contained callback to execute in the extension service worker. Values from the caller's
  lexical scope are not captured.
</ParamField>

<ParamField path="input" type="Input" optional>
  JSON-serializable input passed as the callback's second argument.
</ParamField>
...
<ParamField path="options" type="ExperimentalBatchOptions" optional>
  ...
  <ParamField path="options.page" type="Page" optional>
    The page supplied to the callback. The active page is used when omitted.
  </ParamField>

  <ParamField path="options.timeout" type="number" optional>
    The overall callback deadline in milliseconds.
  </ParamField>
</ParamField>
...
Browser operations use the worker-local `page` and `context` objects.
...
<ParamField path="source" type="str">
  Self-contained JavaScript callback source. Do not pass source from an untrusted user, webpage,
