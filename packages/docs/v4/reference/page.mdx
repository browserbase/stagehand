...
Subscribe to a Chrome DevTools Protocol event from this page and its page-owned
sessions. The `"console"` alias subscribes to `"Runtime.consoleAPICalled"`.
...
<ParamField path="event" type="Event">
  A generated CDP event name, or the `"console"` alias.
</ParamField>

<ParamField path="listener" type="PageEventListener<Event>">
