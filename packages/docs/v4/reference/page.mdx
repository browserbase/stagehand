...
Return the full HTML serialization of the page's main document, including its doctype.
...
Replace the page's main document with the given HTML and wait for it to load.
...
<ParamField path="html" type="string">
  The HTML markup to load into the page.
</ParamField>
...
<ParamField
  ...
  optional
>
  The load state to wait for. Defaults to `"load"`.
</ParamField>
...
<ParamField path="options.Timeout" type="*int" optional>
  ...
  <ParamField path="options.WaitUntil" type="*LoadState" optional>
    The target load state: `LoadStateLoad`, `LoadStateDOMContentLoaded`, or
    `LoadStateNetworkIdle`. Defaults to `LoadStateLoad`.
  </ParamField>
</ParamField>
