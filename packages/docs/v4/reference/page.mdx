...
const invocation = await tools[0].invoke({ input: { query: "Stagehand" } });
```
...
<ResponseField name="result" type="Promise<WebMCPTool[]>">
  The callable WebMCP tools registered on the page.
</ResponseField>
...
invocation = await tools[0].invoke(input={"query": "Stagehand"})
```
