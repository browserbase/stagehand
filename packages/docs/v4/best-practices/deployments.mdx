...
Never commit secrets. Add variables via the Vercel CLI, then read them in your handler and pass them to `browserbase.launch()` and `Stagehand.create()`. `CRON_SECRET` is the secret the handler compares against the `Authorization` header, and Vercel sends it automatically when a cron job invokes the function:
