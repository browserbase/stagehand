...
Browserbase Cache is a managed, server-side caching layer inside the Stagehand API. It is on by default: Browserbase caches every `act()`, `observe()`, and `extract()` call on its servers, and repeated calls with the same inputs return instantly without consuming any LLM tokens. Set `cache: false` to turn it off.
...
Caching is enabled by default. Pass `cache` on `Stagehand.create()` to set the behaviour for every request made by that instance — `false` opts out, or pass an object to tune the threshold:
