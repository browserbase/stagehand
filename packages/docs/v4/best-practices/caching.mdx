...
Locator-scoped `observe()` and `extract()` calls currently bypass the server-side result cache, because the cache contract is keyed on unscoped requests. Their cache status is `DISABLED` even when instance-level caching is enabled.
...
<Note>
When you set `locator` or `ignoreLocators` on `observe()` or `extract()`, Stagehand skips server-side cache reads and writes for that call.
</Note>
</Accordion>
