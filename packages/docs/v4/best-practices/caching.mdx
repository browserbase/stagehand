...
Caching is enabled by default. Pass `cache` on `Stagehand.create()` to set the behaviour for every request made by that instance. Set it to `false` to opt out, or pass an object to tune the threshold:
