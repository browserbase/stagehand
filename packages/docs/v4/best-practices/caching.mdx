...
Every result carries `metadata.cache`, so you can verify whether a result was served from cache. The status is always present: `HIT`, `MISS`, or `DISABLED` when no cache lookup ran at all. A miss also carries a `missReason`, and a hit carries the tokens it saved.
...
<Warning>
Do not assume two runs with different values share a cache entry. The variables you pass travel to the cache service with the rest of the request, so a different value may produce different key data and miss. Because they are transmitted, pass `cache: false` on any call carrying a credential. See [prompting with variables](/v4/best-practices/prompting-best-practices) for the full caveat.
</Warning>

{/* TODO: this accordion previously promised that the cache key is derived from variable keys rather than values, so that {email: alice} and {email: bob} shared one entry ("prime once and hit forever"). That promise was removed because it could not be verified and the available evidence contradicts it: buildActCacheData transmits the whole options.variables record, and the comment in that file says only key-participating fields are sent. If values are in fact excluded from the key, restore the generalization guidance; if they are not, consider whether this accordion belongs on the caching page at all, since variables would then offer no caching benefit. */}
...
{/* TODO: Model Gateway routing is not yet wired up in the v4 runtime, so a model config with no provider key constructs and init()s fine but throws on the first act()/observe()/extract(): packages/server/services/llmService.ts:29 raises "Direct model inference requires an API key" (the throw is only reachable from actService.ts:250, extractService.ts:137 and observeService.ts:86, never from init). Cached calls short-circuit before inference, so only cache misses fail. Same caveat as v4/configuration/models.mdx:69. Verify before publishing. */}
