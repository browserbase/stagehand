...
Stagehand enables caching by default. Pass `cache` on `Stagehand.create()` to set the behavior for every request from that instance. Set it to `false` to opt out, or pass an object to tune the threshold:
