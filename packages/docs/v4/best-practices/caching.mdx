...
When targeting a specific part of a page, pass a locator to scope the accessibility tree snapshot to that container. This reduces token costs, speeds up inference, and, crucially for caching, means that changes *outside* the container don't affect the cache key.
...
<Note>
When you set a locator, Stagehand hashes only that subtree. If the locator fails to resolve, Stagehand skips caching for that call rather than falling back to the whole page.
</Note>
</Accordion>
