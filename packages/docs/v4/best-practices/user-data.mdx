...
When you need a portable auth snapshot (CI artifacts, sharing cookies across machines) instead of a full Chrome profile, export and restore cookies with `storageState` / `storage_state` / `StorageState`. The JSON shape matches Playwright's cookie list. Stagehand reads cookies over CDP, so the export includes HttpOnly cookies. Stagehand does not export localStorage yet and returns `origins` as an empty list.
