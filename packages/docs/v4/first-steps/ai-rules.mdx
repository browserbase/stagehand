...
console.log(result.metadata.cache?.status); // "HIT", "MISS", or undefined
```
...
print(result.metadata.cache.status if result.metadata.cache else None)  # "HIT", "MISS", or None
```
...
Both `ActionID` and `Cache` are pointers, so check for nil before dereferencing:
...
if result.Metadata.Cache != nil {
	fmt.Println("cache:", result.Metadata.Cache.Status) // stagehand.CacheStatusHIT or stagehand.CacheStatusMISS
}
```
