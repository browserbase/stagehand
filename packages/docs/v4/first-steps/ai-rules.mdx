...
- Construct with one typed configuration dictionary, then `await stagehand.init()`:
...
async def main() -> None:
    stagehand = Stagehand({
        "browser": {
            "type": "local",  # or "browserbase" (needs an API key)
            "headless": True,
        },
        "model": {
...
- `act`, `extract`, and `observe` are methods on `stagehand`, NOT on `page`.
- They run on the active page by default; pass `{"page": page}` as the options dictionary to target
  a specific one.
