...
- `locator`: scopes extraction to one element and its subtree.
- `ignoreLocators`: elements to exclude, such as ads, chrome, and boilerplate.
...
When a page is large, scoping to the relevant region with `locator` (and excluding noise
with `ignoreLocators`) is faster, cheaper, and more accurate than extracting from the
whole document.
...
const {
  ...
  {
    locator: { selector: "#product-summary" },
    ignoreLocators: [{ selector: ".related-products" }],
  },
);
```
...
- **Scope first:** Reach for `locator`/`ignoreLocators` before extracting from a busy
  page.
...
Narrow with `locator`, exclude distractors with `ignoreLocators`, or enable `screenshot`
...
extraction = await stagehand.extract(
    ...
    schema=schema,
    locator=ProtocolLocator(selector="#main-content"),
    ignore_locators=[
        ProtocolLocator(selector="nav"),
        ProtocolLocator(selector=".cookie-banner"),
    ],
    screenshot=True,
    timeout=30_000,
...
- `page`: the page to extract from. Defaults to the active page.
- `locator`: scopes extraction to one element and its subtree.
- `ignore_locators`: elements to exclude, such as ads, chrome, and boilerplate.
- `screenshot`: include a screenshot of the viewport for layout-dependent extraction.
- `timeout`: maximum time in milliseconds.
...
When a page is large, scoping to the relevant region with `locator` (and excluding noise
with `ignore_locators`) is faster, cheaper, and more accurate than extracting from the
whole document.
...
result = await stagehand.extract(
    ...
    schema=Product,
    locator=ProtocolLocator(selector="#product-summary"),
    ignore_locators=[ProtocolLocator(selector=".related-products")],
)
```
...
- **Use descriptions:** Field descriptions disambiguate similar values (for example, which
  price is the sale price).
- **Scope first:** Reach for `locator`/`ignore_locators` before extracting from a busy
  page.
...
Narrow with `locator`, exclude distractors with `ignore_locators`, or enable `screenshot`
when the correct value is clear visually but ambiguous in the DOM.
