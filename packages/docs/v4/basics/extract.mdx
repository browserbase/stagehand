...
result, err := stagehand.Extract[repository](ctx, client, "extract the repository name", &stagehand.StagehandClientExtractOptions{
	ExtractOptions: stagehand.ExtractOptions{
		Model:   &model,
		Timeout: &timeout,
		Locator: &stagehand.Locator{Selector: "//header"}, # Focus on a specific area
	},
})
if err != nil {
	return err
...
Pass a locator to `extract` to target a specific element on the page.
<Tip>
...
const { data: tableData } = await stagehand.extract(
  ...
  {
    locator: { selector: "xpath=/html/body/div/table/" },
  },
);
```
</View>
...
table_data = (await stagehand.extract(
    "Extract the values of the third row",
    TableRow,
    locator=ProtocolLocator(selector="xpath=/html/body/div/table/"),
)).data
```
</View>
...
tableData, err := stagehand.Extract[tableRow](
	...
	&stagehand.StagehandClientExtractOptions{
		ExtractOptions: stagehand.ExtractOptions{
			Locator: &stagehand.Locator{Selector: "xpath=/html/body/div/table/"},
		},
	},
)
```
...
article = (await stagehand.extract(
    ...
    Article,
    ignore_locators=[
        ProtocolLocator(selector=".ad"),
        ProtocolLocator(selector=".newsletter-modal"),
        ProtocolLocator(selector="nav.related-posts"),
    ],
)).data
```
</View>
...
<Note>
  An ignore locator without `nth` removes all matches and their descendants. Set `nth` to exclude only that zero-based match. The focus `locator` scopes extraction to one resolved subtree.
</Note>
