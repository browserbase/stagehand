...
<Note>
  Ignored locators remove all matches for each locator, along with each matched node's descendants. The locator option still scopes extraction to a single resolved subtree.
</Note>
