...
const actions = await stagehand.observe("Find form fields", {
  locator: { selector: "form#signup" },
  ignoreLocators: [{ selector: ".advertisement" }],
  timeout: 15_000,
...
- `locator`: scopes observation to one element.
- `ignoreLocators`: elements to exclude.
...
<Note>
  An `ignoreLocators` entry without `nth` removes every match and its descendants. Set `nth` to
  exclude only that zero-based match. `locator` scopes observation to a single resolved subtree.
</Note>
...
- **Scope busy pages:** `locator` and `ignoreLocators` cut noise and speed up planning.
...
Scope observation to the relevant region with `locator`, and exclude noise with
`ignoreLocators`.
...
actions = await stagehand.observe(
    ...
    locator=ProtocolLocator(selector="form#signup"),
    ignore_locators=[ProtocolLocator(selector=".advertisement")],
    timeout=15_000,
...
- `page`: the page to observe. Defaults to the active page.
- `locator`: scopes observation to one element.
- `ignore_locators`: elements to exclude.
- `timeout`: maximum time in milliseconds.
...
<Note>
  An `ignore_locators` entry without `nth` removes every match and its descendants. Set `nth` to
  exclude only that zero-based match. `locator` scopes observation to a single resolved subtree.
</Note>
...
- **Observe to verify, act to do:** Use `observe()` to confirm the right element exists,
  then run `act()` with the same intent.
- **Scope busy pages:** `locator` and `ignore_locators` cut noise and speed up planning.
...
if tables.data:
    pricing = await stagehand.extract(
        ...
        schema=Pricing,
        locator=ProtocolLocator(selector=tables.data[0].selector),
    )
    print(pricing.data)
```
