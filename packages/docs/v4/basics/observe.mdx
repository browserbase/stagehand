...
const { data: actions } = await stagehand.observe("find navigation links", {
  ...
  timeout: 30000,
  locator: { selector: "//header" }, // Focus on a specific area
});
```
</View>
...
result = await stagehand.observe(
    ...
    timeout=30000,
    locator=ProtocolLocator(selector="//header"),  # Focus on a specific area
)
```
</View>
...
result, err := client.Observe(ctx, &instruction, &stagehand.StagehandClientObserveOptions{
	ObserveOptions: stagehand.ObserveOptions{
		Model:   &model,
		Timeout: &timeout,
		Locator: &stagehand.Locator{Selector: "//header"},
	},
})
if err != nil {
	return err
...
You can also exclude specific nodes, including their descendant nodes, with `ignoreLocators`.

<View title="TypeScript" icon="js">
```typescript
const { data: actions } = await stagehand.observe("find the main call-to-action buttons", {
  ignoreLocators: [
    { selector: "//aside[contains(@class, 'promo-rail')]" },
    { selector: "//div[@id='floating-chat-launcher']" },
    { selector: "//section[@aria-label='recommended articles']" },
  ],
});
```
</View>
...
<View title="Python" icon="python">
```python
from stagehand import ProtocolLocator


result = await stagehand.observe(
    "find the main call-to-action buttons",
    ignore_locators=[
        ProtocolLocator(selector="//aside[contains(@class, 'promo-rail')]"),
        ProtocolLocator(selector="//div[@id='floating-chat-launcher']"),
        ProtocolLocator(selector="//section[@aria-label='recommended articles']"),
    ],
)
```
</View>
...
result, err := client.Observe(ctx, &instruction, &stagehand.StagehandClientObserveOptions{
	ObserveOptions: stagehand.ObserveOptions{
		IgnoreLocators: []stagehand.Locator{
			{Selector: "//aside[contains(@class, 'promo-rail')]"},
			{Selector: "//div[@id='floating-chat-launcher']"},
			{Selector: "//section[@aria-label='recommended articles']"},
		},
...
<Note>
  An ignore locator without `nth` removes all matches and their descendants. Set `nth` to exclude only that zero-based match. The focus `locator` scopes observation to one resolved subtree.
</Note>
...
// Open a second page in the same browser context
productsPage, err := browserContext.NewPage(ctx, nil)
if err != nil {
	return err
}
if _, err := productsPage.Goto(ctx, "https://www.example.com/products", nil); err != nil {
	return err
}
...
Use `observe()` to find a container, then wrap its selector in an `extract()` locator. Extraction then sees only that subtree instead of the whole page, so token usage falls in proportion to how much of the page you exclude.
...
const { data: pricing } = await stagehand.extract(
  ...
  PricingSchema,
  { locator: { selector: table.selector } },
);
...
pricing = (await stagehand.extract(
    "extract all pricing tiers",
    Pricing,
    locator=ProtocolLocator(selector=observed.data[0].selector),
)).data
...
pricing, err := stagehand.ExtractAs[pricingTiers](
	...
	&stagehand.StagehandClientExtractOptions{
		ExtractOptions: stagehand.ExtractOptions{
			Locator: &stagehand.Locator{Selector: observed.Data[0].Selector},
		},
	},
)
if err != nil {
