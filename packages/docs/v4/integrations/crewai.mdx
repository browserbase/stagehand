---
title: "CrewAI"
description: "Give a CrewAI agent persistent Stagehand Code Mode tools over MCP/stdio."
---

The CrewAI integration connects a Python agent to the shared Stagehand Code Mode server over MCP/stdio. One server process owns the browser, so page state survives across `run`, `snapshot`, and `screenshot` calls.
...
| `OPENAI_API_KEY` | Credential for the example's default agent model. It is not forwarded to the MCP child. |
