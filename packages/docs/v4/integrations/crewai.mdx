...
The CrewAI integration connects a Python agent to the shared Stagehand integration server over MCP/stdio. One server process owns the browser, so page state survives across `run`, `snapshot`, and `screenshot` calls.
