...
The Vercel AI SDK integration connects an AI SDK tool loop to the shared Stagehand integration server over MCP/stdio. One server process owns the browser, so page state survives across `run`, `snapshot`, and `screenshot` calls.
