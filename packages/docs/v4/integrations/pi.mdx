...
pi ships without built-in MCP by design; extensions register tools directly. The Stagehand integration is a pi extension that binds `run`, `snapshot`, and `screenshot` as native in-process tools backed by one browser session per pi session, without starting an MCP server or bridge process. The tool descriptions, runtime validators, and system prompt come from the shared facade contract, so the tools behave the same as the MCP-based integrations.
