---
title: "Eve"
description: "Give Eve, Vercel's framework for building agents, native Stagehand Code Mode tools backed by one durable browser session."
---

[Eve](https://vercel.com/eve) is Vercel's framework for building agents with Markdown instructions and skills, TypeScript tools, and durable execution. The Stagehand integration binds `run`, `snapshot`, and `screenshot` as native in-process tools without starting an MCP server or bridge process.
