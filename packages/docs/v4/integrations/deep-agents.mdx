---
title: "Deep Agents"
description: "Give LangChain Deep Agents persistent Stagehand tools locally or in a managed deployment."
---
...
```bash
uv add deepagents langchain-mcp-adapters langchain-openai
```

Configure your agent’s model credentials as usual. The browser uses local Chrome unless you export `BROWSERBASE_API_KEY`. The Deep Agents host forwards the `STAGEHAND_*` and `BROWSERBASE_*` variables to the MCP child, including any separate Stagehand model configuration. Your agent’s model-provider credential stays in the host.
...
async def main():
    ...
    # Keep one explicit session: browser state and snapshot IDs are stateful.
    async with client.session("stagehand") as session:
        tools = await load_mcp_tools(session)
        agent = create_deep_agent(model="openai:gpt-5.6-luna", tools=tools)
...
The Stagehand facade MCP server also isolates Stagehand’s dependencies from your Python agent. Do not replace `client.session()` with stateless per-call tool loading: each new server process would lose the current browser. The Python-native and Managed Deep Agents examples below remain available when you need those deployment models.
