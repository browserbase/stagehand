...
<Info>
Integration distribution varies by host. The Eve native extension is being prepared as `@browserbasehq/eve` 0.2; its guide identifies the pending release and source-testing path. The remaining examples and shared Core package currently run from the monorepo.
</Info>
