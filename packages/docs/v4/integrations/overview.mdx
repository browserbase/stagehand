---
title: "Code Mode integrations"
sidebarTitle: "Overview"
description: "Connect CrewAI, Deep Agents, Eve, Mastra, or the Vercel AI SDK to a persistent Stagehand browser."
---

Stagehand's Code Mode integrations give an agent one persistent browser and three tools: `run`, `snapshot`, and `screenshot`. Use an integration when you want an agent framework to decide how to navigate and interact while Stagehand owns the browser session.

<Info>
These integrations are experimental and currently run from the Stagehand monorepo. The adapters and shared Code Mode package are not published as standalone packages.
</Info>

## Choose an integration

<CardGroup cols={2}>
  <Card title="CrewAI" icon="users" href="/v4/integrations/crewai">
    Connect a Python CrewAI agent over MCP/stdio.
  </Card>
  <Card title="Deep Agents" icon="diagram-project" href="/v4/integrations/deep-agents">
    Run locally over MCP or deploy native tools with Managed Deep Agents.
  </Card>
  <Card title="Eve" icon="bolt" href="/v4/integrations/eve">
    Bind native TypeScript tools directly to an Eve agent.
  </Card>
  <Card title="Mastra" icon="robot" href="/v4/integrations/mastra">
    Connect a Mastra agent over MCP/stdio.
  </Card>
  <Card title="Vercel AI SDK" icon="code" href="/v4/integrations/vercel-ai-sdk">
...
<AccordionGroup>
  <Accordion title="snapshot">
    Read a compact accessibility tree for the active page. Interactive elements include bracketed IDs that can be passed to `run` actions.
...
The private [`core/` workspace package](https://github.com/browserbase/stagehand/tree/main/packages/integrations/core) owns the shared TypeScript tool contract, browser runtime, native bindings, and stdio MCP server used by the TypeScript integrations and CrewAI.
...
CrewAI, Eve, Mastra, and the Vercel AI SDK use the shared TypeScript Code Mode server. They require Node.js 24 or newer and pnpm 11.10.0. CrewAI also requires Python 3.11–3.13 and [uv](https://docs.astral.sh/uv/).
...
```
  ...
  <Step title="Build the shared Code Mode package">
```bash
...
The examples use local Chrome when `BROWSERBASE_API_KEY` is not set. A current Google Chrome installation is required for local mode.
...
| `STAGEHAND_MODEL_NAME` | Optional model for Stagehand AI methods called from TypeScript Code Mode tools. |
| `STAGEHAND_MODEL_API_KEY` | Optional credential for `STAGEHAND_MODEL_NAME`. |

The framework's agent model and Stagehand's optional browser model are separate. The agent model decides which tool to call. A Stagehand model is needed only when JavaScript passed to `run` calls AI methods such as `act`, `extract`, or `observe`.
