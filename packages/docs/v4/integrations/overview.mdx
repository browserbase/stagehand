...
description: "Connect agent frameworks to a persistent Stagehand browser or automate Stripe Checkout with WebMCP."
---

The agent framework integrations give your agent one persistent browser and three tools: `run`, `snapshot`, and `screenshot`. Your agent decides how to navigate and interact while Stagehand manages the browser session.

<Info>
Stagehand ships these experimental agent framework integrations from the monorepo and does not publish the adapters or shared integration package as standalone packages.
</Info>

Use the [Stripe integration](/v4/integrations/stripe) to interact with Checkout through page-provided WebMCP tools. It runs as a separate sample in Stripe’s repository.
