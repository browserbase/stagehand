...
Stagehand blocks HTTP(S) requests to cloud metadata and link-local endpoints by default. `context.setDomainPolicy()` narrows the hosts a session can reach further, and an explicit policy replaces the default posture.
