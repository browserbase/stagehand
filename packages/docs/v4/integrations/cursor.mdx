...
This V1 integration uses Cursor's local runtime. The agent loop and temporary workspace run locally, while model inference remains hosted by Cursor. Cursor Cloud requires a remotely reachable MCP server and is not part of this example.
