...
The package ships a project-scoped `.cursor/mcp.json` that mounts the same facade server in the Cursor CLI. Its `args` path is relative to the package, so start the CLI from that directory:
