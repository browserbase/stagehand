...
The `RETRYABLE` test matters. Without it the loop swallows assertion and logic failures until the deadline, so a genuine bug surfaces 30 seconds later as a timeout. Widen the pattern as you meet new transient messages, and keep rethrowing everything else immediately.
...
- One wait is not Playwright's per-action retry. For a step that races a
  ...
  The RETRYABLE guard is required. Retrying every error swallows assertion and
  logic failures until the deadline, turning a real bug into a timeout. NEVER
  retry act(): a failed act() may already have clicked or submitted. Retry
  observe() instead, then pass its Action to act() once.
