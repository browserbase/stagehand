...
Downloads are configured at launch rather than awaited. `localBrowser.launch()` takes `acceptDownloads` and `downloadsPath`, and `downloadsPath` is required whenever `acceptDownloads` is `true`. There's no download event and no `download.saveAs()`, so watch the directory yourself instead of awaiting the download.
...
- Accessible name precedence is `aria-label`, then the associated label's text, then `alt` on an image, then a button's `value`, then `textContent`, then `title`. Playwright's `name` option is a case-insensitive substring match unless you pass `exact: true`.
...
async function actWhenReady(
  ...
  while (Date.now() < deadline) {
    if ((await locator.count()) > 0 && (await locator.isVisible())) {
      try {
        ...
        // A re-render between the check and the action detaches the element.
  ...
  throw new Error(`Timed out after ${timeout}ms waiting for an actionable element`);
}

await actWhenReady(page, page.locator("#checkout"), (target) => target.click());
```

Retry only the errors that mean "not ready yet": a message matching detached, not visible, no element found, or timed out. Rethrow anything else, or a genuine failure will burn the full timeout before surfacing.
...
- Accessible name precedence: aria-label, then associated label text
  ...
  then a button's value, then textContent, then title. Playwright's name
...
- One wait is not Playwright's per-action retry. For a step that races a
  ...
  Retry only messages that mean "not ready yet": detached, not visible, no
  element found, timed out. Rethrow anything else. NEVER retry act(): a failed
  act() may already have clicked or submitted. Retry observe() instead, then
  pass its Action to act() once.
