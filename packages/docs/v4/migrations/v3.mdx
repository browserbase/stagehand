...
Pass the Zod schema as the second argument, same as the positional-args change above. ArkType works the same way. A raw JSON Schema object does not; wrap a JSON Schema object that follows Draft 2020-12 with `jsonSchema()`. Valibot and Effect need the adapters in [supported TypeScript schema implementations](/v4/reference/stagehand#supported-typescript-schema-implementations).
...
| `extract()` with Zod 3 or a JSON object | Zod 4.2+ from `zod/v4`, or `jsonSchema()` for JSON Schema objects |
...
**Extract validation fails on extra properties.** Stagehand rejects fields that are not declared in an object schema. Add `additionalProperties: true` when the output may contain undeclared fields.
