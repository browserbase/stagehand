...
Pass the Zod schema as the second argument, same as the positional-args change above. ArkType works the same way. A raw JSON Schema object does not; wrap a plain Draft 2020-12 document with `jsonSchema()`. Valibot and Effect need the adapters in [supported TypeScript schema implementations](/v4/reference/stagehand#supported-typescript-schema-implementations).
