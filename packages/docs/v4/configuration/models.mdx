...
const stagehand = await Stagehand.create({
  browser: await browserbase.launch({ apiKey: process.env.BROWSERBASE_API_KEY }),
  model: { modelName: "anthropic/claude-sonnet-5" },
});
```
</Tab>
...
apiKey := os.Getenv("BROWSERBASE_API_KEY")
// No model API key: requests route through Model Gateway
model := stagehand.ModelConfig{ModelName: "anthropic/claude-sonnet-5"}
...
apiKey := os.Getenv("BROWSERBASE_API_KEY")
// No model API key: requests route through Model Gateway
model := stagehand.ModelConfig{ModelName: "google/gemini-3.8-flash"}
...
Commonly used: `google/gemini-3.8-flash`, `google/gemini-3.7-flash`, `google/gemini-3.6-flash`, `google/gemini-3.5-flash`, `google/gemini-3.1-pro-preview`.
...
Commonly used: `anthropic/claude-fable-5-1`, `anthropic/claude-opus-5`, `anthropic/claude-sonnet-5`, `anthropic/claude-haiku-4-5`, `anthropic/claude-sonnet-4-6`.
...
Commonly used: `openai/gpt-6-astra`, `openai/gpt-5.6-sol`, `openai/gpt-5.6-terra`, `openai/gpt-5.6-luna`, `openai/gpt-5.5`, `openai/gpt-5.4`.
...
[Vercel AI Gateway](https://vercel.com/docs/ai-gateway) exposes any of its models through an OpenAI-compatible Chat Completions endpoint. `openAICompatible` calls that endpoint with `fetch`, so you do not install the `openai` package. The model id is the gateway's own id, including models Stagehand does not ship.

```typescript
import { browserbase, openAICompatible, Stagehand } from "@browserbasehq/stagehand";
...
Any other Chat Completions endpoint takes the same helper: pass its model id, base URL, and API key. For an endpoint that is not Chat Completions compatible, use the callback under [Custom models](#custom-models).
</Accordion>
...
Different models excel at different tasks. The picks below follow [Browserbase Benchmark v2](https://www.stagehand.dev/evals).

<Card title="Browser Agent Evals" href="https://www.stagehand.dev/evals" icon="scale-balanced">
  Live accuracy, cost, and speed comparisons across models and harnesses.
</Card>
...
| ----------------- | --------------------------- | ------------------------------------ |
| **Production**    | `openai/gpt-5.6-sol`        | Strong accuracy without a high bill  |
| **Intelligence**  | `openai/gpt-6-astra`        | Best accuracy on hard tasks          |
| **Speed**         | `openai/gpt-5.6-luna`       | Fastest response times               |
| **Cost**          | `openai/gpt-5.6-luna`       | Lowest cost per task                 |
...
- Use the `provider/model` format: `openai/gpt-5.6-sol`. The prefix is required; bare model names are rejected
