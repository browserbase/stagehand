...
<Steps>
<Step title="Install dependencies">
TypeScript uses the built-in `openAICompatible` helper for Chat Completions endpoints. Python and Go use the provider SDK for their callbacks.
...
<Note>
The TypeScript helper calls an OpenAI-compatible Chat Completions endpoint. For a provider that does not support Chat Completions, write a callback as shown under [Custom models](#custom-models).
</Note>
