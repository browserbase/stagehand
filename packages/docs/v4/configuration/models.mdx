...
<Note>
  Omitting `model` from both `browserbase.launch()` and `Stagehand.create()` enables routing for that Stagehand instance. Selection itself happens per call, so one run can use different models at different steps. [Per-call overrides](#per-call-model-overrides) enable selecting a specific model instead of routing it.
</Note>
...
<Note>
  For a Browserbase session default, configure `model` when launching the browser and omit it from `Stagehand.create()`. Stagehand inherits the launch model and records its public name in the session's creation-time metadata; API keys and custom headers remain private to the SDK. The create-level option remains available for local browsers and client-side LLM callbacks, but a model configured there cannot be added to Browserbase metadata after launch. Do not configure a model in both places.
</Note>
