...
<Steps>
<Step title="Use the Stagehand SDK">
The OpenAI-compatible helpers are included in the TypeScript, Python, and Go SDKs. No provider SDK is required.
</Step>
...
<Tab title="TypeScript">
```typescript
import { openAICompatible } from "@browserbasehq/stagehand";
...
from stagehand import Stagehand, browserbase, open_ai_compatible
...
<Tab title="Go">
```go
model := stagehand.OpenAICompatible(stagehand.OpenAICompatibleOptions{
    Model:   "xiaomi/mimo-v2.6-pro",
    BaseURL: "https://ai-gateway.vercel.sh/v1",
...
<Note>
These helpers call an OpenAI-compatible Chat Completions endpoint. For a provider that does not support Chat Completions, write a callback as shown under [Custom models](#custom-models).
</Note>
</Step>

<Step title="Pass the helper to Stagehand">
