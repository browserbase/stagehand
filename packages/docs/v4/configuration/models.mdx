...
<Callout icon="sparkles" color="#FFC107" iconType="regular">
  **(NEW) Automatic Model Router**: Omit `model` and let Browserbase select a model for every `act`, `extract`, and `observe` call. One key, one bill, no provider accounts needed.
</Callout>
