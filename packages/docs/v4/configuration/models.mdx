...
For a Chat Completions endpoint, the TypeScript SDK can call it directly. `openAICompatible` uses `fetch`, so you do not install the `openai` package. Python and Go still use the callback below.
...
Write the callback yourself when the endpoint does not implement Chat Completions, or when you need to map the request differently.
