...
Browserbase session management and Stagehand managed services use separate APIs. Both default to the production service, and neither URL is read from an environment variable. Pass `baseUrl` to Browserbase and `apiUrl` to Stagehand when testing another deployment. Use the service origin without `/v1`.
...
The Browserbase URL controls extension, session, and connection requests made by `browserbase.launch()` or `browserbase.connect()`. The Stagehand URL controls Model Gateway and managed-cache requests made by the browser runtime.
