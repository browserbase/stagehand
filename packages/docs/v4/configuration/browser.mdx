...
### Use an existing Browserbase client

In TypeScript, pass an existing Browserbase SDK client when you need Stagehand's session and extension lifecycle calls to use that client's configuration. This is useful for applications that centralize Browserbase client setup or provide a custom Browserbase endpoint.
...
When `client` is provided, it determines the Browserbase management endpoint; `baseUrl` is not used for those calls. `apiKey` still configures Stagehand runtime authentication.

Python accepts an existing `AsyncBrowserbase` client in both `browserbase.launch()` and `browserbase.connect()`:
...
async with AsyncBrowserbase(api_key=os.environ["BROWSERBASE_API_KEY"]) as client:
...
The supplied client's configuration controls management requests, ignoring `base_url` for those calls. `api_key` still authenticates the Stagehand runtime. Stagehand borrows the client and never closes it; keep it open until browser cleanup finishes, then close it yourself.

Go exposes a reusable management client over Stagehand's built-in Browserbase HTTP implementation. Pass it as `Client` in `BrowserbaseLaunchOptions` or `BrowserbaseConnectOptions`:

```go
httpClient := &http.Client{Timeout: 60 * time.Second}
managementClient, err := stagehand.NewBrowserbaseClient(stagehand.BrowserbaseClientOptions{
    APIKey:     os.Getenv("BROWSERBASE_API_KEY"),
...
`BrowserbaseClientOptions` configures management credentials (`APIKey`), the endpoint (`BaseURL`), an optional caller-owned `HTTPClient`, and an optional `MaxRetries` pointer (zero disables retries). A supplied `Client` takes precedence over the browser factory's `BaseURL`; the factory's `APIKey` still authenticates the Stagehand runtime. The client can be reused across browsers, and closing a browser does not close its HTTP transport. When `Client` is omitted, existing behavior is unchanged.
