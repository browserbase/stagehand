...
<Note>
Stagehand provisions its own browser extension on every Browserbase session, so `browserbase.launch()` uploads that extension to your Browserbase account, starts the session against it, and deletes it again when you call `browser.close()`.
</Note>
