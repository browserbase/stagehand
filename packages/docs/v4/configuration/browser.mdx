...
<Note>
When `browserbase.launch()` creates a session without an `extensionId`, it requests the Stagehand extension that ships with Browserbase browsers by adding `"stagehand"` to `browserSettings.extensions`. Any built-in extensions you list are kept in order. Nothing is uploaded or deleted. If you pass your own `extensionId`, it is forwarded untouched and Stagehand attaches to that build over the session's public CDP endpoint instead.
</Note>

### Alternative: Browserbase SDK

If you prefer to manage sessions directly, create the session with the Browserbase SDK and hand Stagehand the resulting session through `browserbase.connect()`. The session you create must include `"stagehand"` in `browserSettings.extensions`: `browserbase.connect()` assumes the session opted into the built-in extension unless you pass an `extensionId` for an extension you uploaded yourself.
...
<Tab title="Go">
```go
// Create the session with your Browserbase client of choice. The create request
// must set browserSettings.extensions to include "stagehand". Then attach by
...
<Note>
If you created the session with your own uploaded extension instead, pass its ID as `extensionId` (TypeScript), `extension_id` (Python), or `ExtensionID` (Go) to `connect()` so Stagehand attaches to that build over the session's public CDP endpoint. There is no automatic fallback: if the session neither opted into the built-in extension nor was given an `extensionId`, `Stagehand.create()` fails.
</Note>
