...
TypeScript, `extension_id` in Python, or `ExtensionID` in Go. When omitted, Stagehand attempts to
load its bundled extension; pass the ID explicitly when the connected Chrome build does not support
loading an unpacked extension over CDP.
