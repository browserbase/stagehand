...
Browserbase session management and Stagehand managed services use separate APIs. Both default to the production service, and neither URL is read from an environment variable. Pass `apiUrl` explicitly when testing another deployment. Use the service origin without `/v1`.
