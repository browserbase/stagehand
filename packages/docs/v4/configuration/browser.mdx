...
When the existing Browserbase session uses a specific Stagehand extension, identify it with
`extensionId` in TypeScript, `extension_id` in Python, or `ExtensionID` in Go. Omit this option to
discover a compatible preloaded Stagehand extension automatically.
...
When the browser already has the Stagehand extension installed, identify it with `extensionId` in
TypeScript, `extension_id` in Python, or `ExtensionID` in Go. If you omit this option, Stagehand
attempts to load its bundled extension; pass the ID explicitly when the connected Chrome build does
not support loading an unpacked extension over CDP.
