...
<Tab title="TypeScript">
```typescript
import type { StagehandLog } from "@browserbasehq/stagehand";

// Simple logger without parsing (for basic console output)
const simpleLogger = (log: StagehandLog) => {
...
<Tab title="Python">
```python
from stagehand import StagehandLog

# Simple logger without parsing (for basic console output)
def simple_logger(log: StagehandLog) -> None:
...
<Tab title="Python">
```python
from stagehand import StagehandLog, StagehandLogData, StagehandLogLevel
