...
All logging is configured through a single `logging` option on the constructor: a level, an output format, and a callback that receives each record. Use the language selector to see the exact shape your SDK accepts.
...
testing = Stagehand(
    browser="local",
    logging={"level": "warn", "on_log": your_test_logger},
    # rest_of_your_configuration...
)
```
</View>
...
client := stagehand.New(stagehand.StagehandClientInitParams{
	...
	// Filter and format inside the callback when you forward records yourself.
	Logging: &stagehand.StagehandClientLoggingConfig{
		OnLog: func(entry stagehand.StagehandLog) {},
	},
})
