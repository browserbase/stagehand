...
// Testing: forward warnings and errors without routine console output
const testing = { level: "warn", console: false, onLog: yourTestLogger } as const;
...
# Testing: forward warnings and errors without routine console output
testing = {"level": "warn", "console": False, "on_log": your_test_logger}
...
By default, eligible logs are written to stderr and also delivered to your configured callback. Anything your own code can write to is a valid callback destination. To forward records to an observability platform, see [External logging platforms](#external-logging-platforms).

`logging.console` (`Logging.Console` in Go) defaults to `true`. Set it to `false` to suppress routine log output to stderr while continuing to receive structured events through your callback:
...
const stagehand = await Stagehand.create({
  browser,
  logging: { level: "info", console: false, onLog: yourProductionLogger },
});
```
</Tab>
<Tab title="Python">
...
stagehand = await Stagehand.create(
    browser=browser,
    logging={"level": "info", "console": False, "on_log": your_production_logger},
)
```
</Tab>
<Tab title="Go">
...
defer func() { err = errors.Join(err, client.Close(ctx)) }()
```
Go uses `*bool`: nil preserves output, and a pointer to false disables it. The value is copied when the client is created.
</Tab>
</Tabs>

The level filters both console output and callbacks; `level: "off"` disables both. `format` controls only console rendering and does not change the callback's structured event. Without a log callback, disabling console output discards routine log events. This option is SDK-local in all three languages and does not change the runtime's log level.

<Note>
Callback-failure diagnostics retain their existing behavior even when routine console output is disabled: TypeScript reports thrown errors and rejected promises, Python reports exceptions from synchronous or awaited callbacks, and Go reports recovered callback panics. These diagnostics still go to stderr, and later notifications can continue. The option does not guarantee complete stderr silence.
</Note>
...
Your own callback receives every log record that passes the level filter. By default, those records also appear in the console output.
...
<Note>
The callback receives every record that passes the level gate, so do any finer filtering inside it. The level gates both destinations and `off` silences both. Keep the level at the lowest severity you want to receive. Set `console: false` (`Console: &consoleOutput` with `consoleOutput := false` in Go) to suppress routine console output while preserving callbacks; callback-failure diagnostics still go to stderr.
</Note>
</Tab>
</Tabs>
...
client, err := stagehand.Create(ctx, stagehand.CreateOptions{
	...
	Logging: &stagehand.StagehandClientLoggingConfig{
		Level:  stagehand.StagehandClientLogLevelInfo,
		Format: stagehand.StagehandClientLogFormatPretty,
		Console: nil, // default: true; use a pointer to false to suppress routine output
...
| console (`Console` in Go) | `true` | Whether to render routine logs to stderr; callback-failure diagnostics are still printed |
| log callback | none | Receives every record that passes the level filter; console output is also enabled by default |
