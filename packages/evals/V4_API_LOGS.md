...
v4 consistently returns a _plausible but different_ element
...
### 16. AI methods moved from Page to Stagehand between spike commits (breaking)

v4-spike #62 ("Move AI methods to Stagehand", `17c76a9`) relocated
`act`/`extract`/`observe` from `Page` to the `Stagehand` client. Every
bench-v4 task ported at commit `2689384` (all calling `page.act(...)` etc.)
broke at runtime with `page.act is not a function` — while **typecheck stayed
green**, because the gitignored `.v4-sdk-types/` declaration shim is a
point-in-time snapshot that had not been regenerated. Fixed 2026-07-23:
regenerated the shim at current HEAD and migrated all bench-v4 tasks (plus
the new replay harness) to `stagehand.act/extract/observe`.
**Impact:** any consumer pinned to generated types (or docs/examples) can
silently drift from the SDK across spike commits; the type boundary gives no
signal. Process fix on our side: regenerate `.v4-sdk-types/` whenever the
v4-spike checkout moves. Suggestion for the v4 team: publish versioned type
artifacts (or changelog breaking-change markers) so consumers see moves like
Page→Stagehand at compile time.

### 17. Extension runtime requires a fresh `packages/server` build (setup friction)

`stagehand.init()` times out with "Timed out waiting for the Stagehand
extension runtime to become ready" when `v4-spike/packages/server/dist` is
stale or its deps are missing (the SDK loads the extension from that dist).
Every v4-spike example script guards this with
`pnpm --dir packages/server run build` before running; external consumers of
the linked SDK get no such guard and no actionable error. Hit 2026-07-23
after pulling new v4-spike commits (new `zustand` dep, unbuilt dist).
**Impact:** eval runs against a freshly-pulled v4-spike fail at init until
