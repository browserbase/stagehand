...
export default defineBenchV4Task(
  ...
  async ({ debugUrl, sessionUrl, stagehand, page, logger }) => {
    try {
      ...
      // (V4_API_LOGS.md #1), and v4's Locator.click exposes no force option
      ...
      throw new Error(
        "V4 GAP: v4 locator.click has no force option (packages/sdk-ts/src/locator.ts) and no act(observeResult) replay (V4_API_LOGS.md #1); google_flights cannot run on v4",
      );
    } catch (error) {
      return {
        _success: false,
