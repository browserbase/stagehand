...
describe("gradeExternalTrajectory", () => {
  ...
  it("persists raw uncertainty and captured evidence without accepting a grade", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "failed-verifier-evidence-"));
    process.env.VERIFIER_PERSIST_TRAJECTORIES = "1";
    mockState.evaluationResult = {
      outcomeSuccess: false,
      processScore: 0,
      findings: [{ category: "verifier_uncertainty", summary: "provider unavailable" }],
    };
    try {
      ...
      const dir = String(result.trajectoryDir);
      expect(
  ...
  const trajectory = {
    ...
    steps: Array.from({ length: 3 }, () => ({
      ...
      actionArgs: {},
      reasoning: "",
      agentEvidence: { modalities: [{ type: "text", content: "captured page evidence" }] },
      probeEvidence: {},
      toolOutput: { ok: true, result: "captured page evidence" },
    })) as TrajectoryStep[],
    status: "complete",
    finalAnswer: "done",
    usage: {},
  ...
  it("persists the gated verdict at the top level of scores/result.json", async () => {
    ...
    try {
      ...
      expect(result.verifierError).toBeUndefined();
      expect(result.outcomeGates).toEqual(["no_final_answer"]);
