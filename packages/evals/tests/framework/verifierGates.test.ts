...
describe("applyVerdictGates — outcome gates", () => {
  ...
  it("preserves a supported completion when execution later ended in error", () => {
    const gates = applyVerdictGates({
      evaluation: passingJudge,
...
describe("strictProcessScore", () => {
  it("reports blocker wording without inferring missing evidence", () => {
    ...
    expect(blockedCriteria).toBe(0);
    expect(perCriterion?.[1]?.blockerMentioned).toBe(true);
  ...
  it("excludes not-applicable criteria and zeros explicitly unsupported work", () => {
    const blocked = "Access Denied wall at step 0. Full credit due to uncontrollable blocker.";
    const { strict, blockedCriteria } = strictProcessScore({
      perCriterion: [
        criterion("locate", 2, 2, blocked, { evidenceInsufficient: true }),
        criterion("add to cart", 3, 3, blocked, { evidenceInsufficient: true }),
...
describe("evidence rows from the 2026-08-31 audit", () => {
  ...
  it("a0a18ca6 (Macy's bot wall): processScore 1.0 becomes 0 strict", () => {
    ...
    expect(gates.processScoreStrict).toBe(1);
    expect(gates.blockedCriteria).toBe(0);
    expect(gates.perCriterion?.every((c) => c.blockerMentioned)).toBe(true);
    expect(gates.outcomeSuccess).toBe(false);
  });
