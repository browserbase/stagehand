...
export async function waitForKey(
  ...
  return new Promise((resolve) => {
    ...
    const onData = (chunk: Buffer): void => {
      // A chunk can carry several keys (e.g. a stray char + Enter): scan it all.
      let other = false;
      for (const byte of chunk) {
        if (byte === 0x0d || byte === 0x0a) return settle("return");
        if (byte === 0x1b || byte === 0x03) return settle("escape"); // Ctrl+C — skip listener flips aborted
        other = true;
      }
      if (other) settle("other");
    };
