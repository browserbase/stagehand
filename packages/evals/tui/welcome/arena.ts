...
import { runInside } from "./dialogue.js";
...
function rankLanes(lanes: Lane[]): Ranked[] {
  return (
    [...lanes]
      // accuracy (pass before fail), then speed, then cost — the podium's three columns
      .sort((a, b) =>
        a.passes !== b.passes
          ? a.passes
            ? -1
            : 1
          : a.totalMs - b.totalMs || a.costUsd - b.costUsd,
      )
...
async function flow(ctx: WelcomeRunContext): Promise<WizardOutcome> {
  ...
  const w = canvasWidth();
  // The intro owns its keys; our skip listener arms only after it returns,
  // so an Esc inside the intro never skips the run as well.
  ...
  try {
    ...
    ruleHeader("The run", { eyebrow: "three models · one real task" });
    // The countdown and the run share one slot: the numbers pop in the
    // middle of the (still empty) lanes region, then the lanes repaint over
    // them — no blank band between the two.
    const raceBlock = new LiveBlock();
    const raceH = raceHeight(lanes);
    for (const label of ["3", "2", "1", "go"]) {
      ...
      await animate({
        ...
        block: raceBlock,
        draw: (t) => {
          const cv = new Canvas(w, raceH);
          const k = ease.outBack(t);
          const cx = Math.floor(w / 2);
          const cy = Math.floor(raceH / 2);
  ...
  // 6. Look inside — the winner's run, narrated by the agent (clack owns stdin
  // for the question; the transcript gets its own skip listener).
  if (process.stdin.isTTY && process.stdout.isTTY) {
    setCursorHidden(false);
    const pick = await clack.select({
      message: "Want to see how the winner got there?",
      options: [
        {
          value: "inside",
          label: "Step inside the winning run",
          hint: "the agent narrates it, step by step",
        },
        { value: "skip", label: "Skip to the prompt" },
      ],
      initialValue: "inside",
    });
    setCursorHidden(true);
    if (clack.isCancel(pick)) return cancelled();
    // clack repaints its closed frame a tick after resolving — let it finish
    // before we print, or our header lands inside its frame.
    await sleep(80);
    if (pick === "inside") {
      const inside = listenForSkip();
      try {
        await runInside(cs, inside.signal);
        if (inside.signal.aborted) return cancelled();
      } finally {
        inside.release();
      }
    }
