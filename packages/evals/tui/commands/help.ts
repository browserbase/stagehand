...
export function printRunHelp(): void {
  print([
    ...
    "",
    row(`${cyan("--harness")} ${dim("<name>")}`, `Bench harness ${gray("(stagehand executable; claude_code/codex dry-run)")}`),
    row(`${cyan("--agent-mode")} ${dim("<mode>")}`, `Single Stagehand agent mode ${gray("(dom | hybrid | cua)")}`),
    row(`${cyan("--agent-modes")} ${dim("<csv>")}`, `Stagehand mode matrix ${gray("(dom,hybrid,cua)")}`),
...
export function printConfigHelp(): void {
  print([
    ...
    row(`${cyan("set")} ${dim("<key> <value>")}`, `Set a default ${gray("(env/trials/concurrency/provider/model/api/verbose/agentModes)")}`),
