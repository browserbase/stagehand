...
export class RubricCache {
  ...
  async read(taskSpec: TaskSpec): Promise<Rubric | undefined> {
    ...
    if (!parsed.rubric?.items?.length) {
      ...
      console.warn(`[rubric-cache] cached rubric for ${taskSpec.id} has no items; regenerating`);
      return undefined;
    }
  ...
  async write(taskSpec: TaskSpec, rubric: Rubric): Promise<void> {
    if (!rubric?.items?.length) {
      ...
      console.warn(`[rubric-cache] refusing to cache empty rubric for ${taskSpec.id}`);
      return;
    }
