...
export async function prepareLLMExposure(
  ...
  const resolvedProfile =
    startupProfile ??
    (env === "BROWSERBASE" ? "runner_provided_browserbase_cdp" : "runner_provided_local_cdp");
  if (
  ...
  try {
    ...
    const connectedBrowser = await chromium.connectOverCDP(target.providedEndpoint.url, {
