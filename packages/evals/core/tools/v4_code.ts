...
class V4LocatorHandle implements CoreLocatorHandle {
  ...
  async type(text: string, opts?: { delay?: number }): Promise<void> {
    await this.locator.type(text, opts?.delay ? { delay: opts.delay } : undefined);
  }
...
class V4PageHandle implements CorePageHandle {
  ...
  async reload(opts?: NavOpts): Promise<void> {
    await this.page.reload(opts?.waitUntil ? { waitUntil: opts.waitUntil } : undefined);
    await this.refreshUrl();
  }
  ...
  async goBack(opts?: NavOpts): Promise<boolean> {
    await this.page.goBack(opts?.waitUntil ? { waitUntil: opts.waitUntil } : undefined);
    const before = this.lastUrl;
    await this.refreshUrl();
    return this.lastUrl !== before;
  }
  async goForward(opts?: NavOpts): Promise<boolean> {
    await this.page.goForward(opts?.waitUntil ? { waitUntil: opts.waitUntil } : undefined);
    const before = this.lastUrl;
    await this.refreshUrl();
    return this.lastUrl !== before;
  }
  ...
  async click(targetOrX: string | ActionTarget | number, maybeY?: number): Promise<void> {
    if (typeof targetOrX === "number" || typeof maybeY === "number") {
      throw new Error("v4_code: coordinate clicks are unsupported (v4 is DOM-only)");
    }
    await this.page.locator(targetSelector(targetOrX)).click();
  }

  async hover(targetOrX: string | ActionTarget | number, maybeY?: number): Promise<void> {
    if (typeof targetOrX === "number" || typeof maybeY === "number") {
      throw new Error("v4_code: coordinate hover is unsupported (v4 is DOM-only)");
    }
  ...
  async scroll(_x: number, _y: number, deltaX: number, deltaY: number): Promise<void> {
    ...
    await this.page.evaluate(({ dx, dy }: { dx: number; dy: number }) => window.scrollBy(dx, dy), {
      dx: deltaX,
      dy: deltaY,
    });
  }
  ...
  async type(
    ...
    await this.page.locator(targetSelector(target as string | ActionTarget)).type(maybeText);
  }
  ...
  async press(
    ...
    await this.page.locator(targetSelector(target as string | ActionTarget)).click();
    await this.page.keyPress(maybeKey);
  }
}
...
export class V4CodeTool implements CoreTool {
  ...
  async start(input: ToolStartInput): Promise<ToolStartResult> {
    ...
    const init = await initV4({
      ...
      configOverrides: {
        env: input.startupProfile === "tool_create_browserbase" ? "BROWSERBASE" : "LOCAL",
      },
    });
    ...
    return {
      ...
      metadata: {
        ...
        connectionMode:
          input.startupProfile === "tool_create_browserbase" ? "browserbase_native" : "launch",
      },
    };
  }
}
...
export async function prepareLLMExposure(
  ...
  const resolvedProfile =
    startupProfile ?? (env === "BROWSERBASE" ? "tool_create_browserbase" : "tool_launch_local");
  if (resolvedProfile !== "tool_launch_local" && resolvedProfile !== "tool_create_browserbase") {
  ...
  const v4 = await initV4({
    ...
    modelName: SURFACE_MODEL,
    configOverrides: {
      env: resolvedProfile === "tool_create_browserbase" ? "BROWSERBASE" : "LOCAL",
    },
  });
